...
if (root) {
  ...
  function downloadPlaceholder() {
    if (platform() === "windows") return "F:/Nooklet/Downloads";
    if (platform() === "macos") return "/Volumes/FastDisk/Nooklet";
  ...
  function prepareRow(row, index) {
    ...
    if (!input || !select || !remove || !error) return;

    const number = index + 1;
    if (!input.id) input.id = `drive-path-${number}`;
  ...
  function showErrors(errors) {
    ...
    for (const item of errors) {
      ...
      if (item.field === "quick-download-path") {
        ...
        const message = field.closest("[data-drive-row]")?.querySelector("[data-drive-error]");
        if (message) {
  ...
  const detectedValue = detectedPlatform.includes("mac")
    ...
    : detectedPlatform.includes("linux")
      ...
      : detectedPlatform.includes("win")
        ? "windows"
        : "";
  const detectedRadio = form.querySelector(
    `input[name="platform"][value="${detectedValue}"]`,
  );
  if (detectedRadio) detectedRadio.checked = true;
  refreshRows();
  ...
  form.addEventListener("input", () => {
    clearErrors();
    if (generatedCommand) clearResult("Folders changed. Generate a fresh setup command.");
  });
