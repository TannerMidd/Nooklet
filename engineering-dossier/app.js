(() => {
    ...
    function applyTheme(mode, persist = true) {
        ...
        if (themeButton) {
            ...
            themeButton.setAttribute("aria-label", `Color theme: ${mode}. Change to ${nextMode}.`);
        ...
        if (themeColor) {
            themeColor.setAttribute(
                "content",
                resolvedTheme(mode) === "light" ? "#f6f1e9" : "#0f0e0d",
            );
        }
    ...
    if (capacityModel) {
        ...
        function updateCapacityModel() {
            if (
                !(freeInput instanceof HTMLInputElement) ||
            ...
            const visibleActiveGiB = Math.min(
                Math.max(0, freeGiB - visibleFloorGiB),
                activeReservationGiB,
            );

            if (freeOutput) {
                freeOutput.textContent = formatGiB(freeGiB);
            }

            if (activeOutput) {
                activeOutput.textContent = formatGiB(activeReservationGiB);
            }

            if (resultValue) {
                resultValue.textContent = formatGiB(maximumNewGiB);
            }

            if (activeReserveValue) {
                activeReserveValue.textContent = formatGiB(activeReservationGiB);
            }

            if (archiveValue) {
                archiveValue.textContent = formatGiB(maximumNewGiB);
            }

            if (unpackValue) {
                unpackValue.textContent = formatGiB(maximumNewGiB);
            }
            ...
            try {
                ...
                if (copyStatus) {
                    copyStatus.textContent =
                        "Copy was unavailable. Select the command text instead.";
                }
            }
