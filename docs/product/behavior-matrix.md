...
| Built-in library management | Users can configure TV and movie libraries, root paths, and default request behavior in Nooklet. | Library and path writes use typed workflows with server-side authorization, path validation, audit events, and normalized persistence; no generic settings endpoint owns library state. | media-library |
