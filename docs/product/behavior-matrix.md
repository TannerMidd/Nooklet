...
| Indexer management | Users can view, configure, test, and remove Sonarr/Radarr indexers without leaving Nooklet. | Each operation is a typed workflow (`save-arr-indexer`, `delete-arr-indexer`, `test-arr-indexer`) gated on a verified connection; indexers stay sourced from the upstream service and are not duplicated in Nooklet's database; field values are never persisted in audit payloads. | service-connections |
