...
| YouTube search or initialization fails                                    | yt-dlp/tool readiness, outbound YouTube access, or unsupported/private URL | Paste a recognized public URL, then inspect authenticated health for tool versions and source-sync failure detail.                            |
| YouTube transfer cannot merge or publish                                  | ffmpeg, `YOUTUBE_WORK_DIR`, destination mapping, or containment failure    | Verify `FFMPEG_PATH`, work/destination write access, the active YouTube path, and that no destination component became a symlink.             |
| YouTube transfer is active but makes no progress                          | Extractor/network stall or blocked work filesystem                         | Inspect Activity and the authenticated runner heartbeat; a genuinely stalled active transfer degrades health and retains durable retry state. |
