...
5. Nooklet automatically closes `/bootstrap` as soon as the administrator exists and refuses later bootstrap attempts.
6. Confirm you can sign in and that `/api/health` is healthy.

The normal setup is complete at this point. For optional defense-in-depth, remove the entire `BOOTSTRAP_TOKEN=...` line from `.env`, save the file, and recreate the container so the token leaves the runtime environment:

    ```console
    docker compose up -d --force-recreate
    ```

This extra cleanup is not required for normal operation. Recheck `docker compose ps` and `/api/health` if you perform it.
...
| Configuration or startup reports a secret error            | Before creating the first admin, confirm all three secrets are present, nonblank, and different. After an admin exists, `/bootstrap` closes automatically; `BOOTSTRAP_TOKEN` may remain configured. If you choose optional defense-in-depth, remove it from `.env` and force-recreate, then confirm `AUTH_SECRET` and `SECRET_BOX_KEY` and recheck YAML indentation. Never post expanded Compose output because it can contain secrets. |
