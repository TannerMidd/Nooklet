...
Service connections live under **Settings -> Connections**. Configure only the integrations needed for the features you plan to use. Instance connections are shared from the administrator account; a regular user can use them but cannot change or verify them. Trakt is the exception: users may connect their own account for personal watch history.
...
Default private-service URLs are examples. From Docker, `localhost` means the Nooklet container, not the host running Plex, Tautulli, or SABnzbd.
...
TMDB is the metadata connection required for setup completion. It powers title identity, search, discovery rails, posters, cast, trailers, genres, and original-language checks. A complete request path also needs either the Usenet or SABnzbd downloader connection described below.
...
SABnzbd remains available for older or already-integrated deployments. Configure the base URL Nooklet can reach and an API key. Verification reads SAB's version and queue state.

Completed files must be visible inside Nooklet. If SAB reports a path that differs from Nooklet's mount, configure `SABNZBD_PATH_MAPPINGS`; see [Storage and path mapping](Storage-and-Path-Mapping).
