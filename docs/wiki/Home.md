...
3. Verify TMDB and either built-in Usenet or SABnzbd.
...
> **Storage rule:** download staging and the final media library are separate filesystems. The built-in downloader admits work using free space on `DOWNLOAD_ENGINE_DIR`, not the final movie or TV drive. Docker users must enter container-side paths such as `/downloads` and `/media/movies`, never host paths such as `F:/Downloads`.
...
Nooklet is an actively developed, single-instance application built around one Next.js process, SQLite, and an in-process background worker. It is not designed for horizontal replicas or a shared network database. Review the [known operational boundaries](Multi-Instance-Deployments) before advanced deployments.
