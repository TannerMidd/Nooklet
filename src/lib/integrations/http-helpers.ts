import { SafeFetchAbortError, safeFetch, type SafeFetchOptions } from "@/lib/security/safe-fetch";
...
export async function fetchWithRetry(
    ...
    for (let attempt = 1; ; attempt += 1) {
        ...
        try {
            ...
            await sleep(jitteredDelayMs(baseDelayMs), init?.signal);
            continue;
        }
        ...
        await sleep(retryAfterMs ?? jitteredDelayMs(baseDelayMs), init?.signal);
    }
}
