...
describe("environment schema", () => {
  ...
  it("rejects non-SQLite database URLs and null-byte paths", () => {
    expect(envSchema.safeParse({ ...validEnvironment, DATABASE_URL: "postgres://db/app" }).success)
      .toBe(false);
    expect(envSchema.safeParse({ ...validEnvironment, DOWNLOAD_ENGINE_DIR: "data\0escape" }).success)
      .toBe(false);
  });
  ...
  it("requires active secrets to be independently generated", () => {
    ...
    if (!result.success) {
      expect(result.error.issues).toEqual(expect.arrayContaining([
        expect.objectContaining({ path: ["SECRET_BOX_KEY"] }),
      ]));
    }
  });
  ...
  it("bounds operational record retention", () => {
    ...
    }).OPERATIONAL_RETENTION_DAYS).toBe(730);
  });
});
...
describe(".env.example parity", () => {
  it("documents every application runtime key and has no stale application keys", () => {
    ...
    expect(
      [...exampleKeys].filter(
        (key) => !runtimeEnvKeys.includes(key as (typeof runtimeEnvKeys)[number])
