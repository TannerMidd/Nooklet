  ...
  it("applies the complete migration set to a fresh database", () => {
    ...
    try {
      ...
      const tables = sqlite
        ...
        .all("download_fulfillments", "download_fulfillment_episodes") as SqliteSchemaObject[];
      expect(tables.map((table) => table.name)).toEqual(
        expect.arrayContaining(["download_fulfillments", "download_fulfillment_episodes"]),
      );

      expect(listColumns(sqlite, "download_fulfillments").map((column) => column.name)).toEqual(
