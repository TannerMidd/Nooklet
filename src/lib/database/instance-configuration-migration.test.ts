...
function listOwnedRows(
  sqlite: Database.Database,
  tableName: "indexers" | "jobs" | "media_libraries" | "media_library_paths" | "service_connections",
...
describe("instance configuration migration", () => {
  it("consolidates configuration and recurring jobs while preserving every collision", () => {
    ...
    try {
      ...
      insertUser(sqlite, { id: "ordinary-user", role: "user", isDisabled: false, createdAt: 10 });
      insertUser(sqlite, { id: "disabled-admin", role: "admin", isDisabled: true, createdAt: 20 });
      insertUser(sqlite, { id: "active-admin-z", role: "admin", isDisabled: false, createdAt: 30 });
      insertUser(sqlite, { id: "active-admin-a", role: "admin", isDisabled: false, createdAt: 30 });
      ...
      `).get()).toEqual({ id: "default", owner_user_id: "active-admin-a" });
      ...
      expect(() => insertServiceConnection(sqlite, {
        id: "duplicate-owner-plex",
        serviceType: "plex",
        scope: "shared",
        ownerUserId: "active-admin-a",
      })).toThrow(/UNIQUE constraint failed/);
      expect(() => insertIndexer(sqlite, {
        id: "duplicate-owner-indexer",
        userId: "active-admin-a",
        name: "Unique A",
      })).toThrow(/UNIQUE constraint failed/);
      expect(() => insertLibrary(sqlite, {
      ...
      const applied0042Before = sqlite.prepare(`
        SELECT COUNT(*) AS count
        FROM __drizzle_migrations
        WHERE created_at = ?
      `).get(migration0042.when) as { count: number };
      const applied0043Before = sqlite.prepare(`
        SELECT COUNT(*) AS count
        FROM __drizzle_migrations
        WHERE created_at = ?
      ...
      const applied0042After = sqlite.prepare(`
        SELECT COUNT(*) AS count
        FROM __drizzle_migrations
        WHERE created_at = ?
      `).get(migration0042.when) as { count: number };
      const applied0043After = sqlite.prepare(`
        SELECT COUNT(*) AS count
        FROM __drizzle_migrations
        WHERE created_at = ?
      `).get(migration0043.when) as { count: number };
  ...
  it("applies recurring-job cleanup after migration 0042 was already recorded", () => {
    ...
    const {
      journal,
      migrationsFolder: migrationsThrough0042,
    } = createMigrationsThrough(sandbox, 42);
    const migration0042 = journal.entries.find((entry) => entry.idx === 42);
    const migration0043 = journal.entries.find((entry) => entry.idx === 43);
    const sqlite = new Database(databasePath);
    ...
    try {
      ...
      insertUser(sqlite, { id: "owner-admin", role: "admin", isDisabled: false, createdAt: 10 });
      insertUser(sqlite, { id: "secondary-user", role: "user", isDisabled: false, createdAt: 1 });
      insertUser(sqlite, { id: "disabled-admin", role: "admin", isDisabled: true, createdAt: 2 });

      migrate(database, { migrationsFolder: migrationsThrough0042 });

      expect(sqlite.prepare(`
        SELECT id, owner_user_id FROM instance_configuration
      `).get()).toEqual({ id: "default", owner_user_id: "owner-admin" });
      expect(sqlite.prepare(`
        SELECT COUNT(*) AS count
        FROM __drizzle_migrations
        WHERE created_at = ?
      `).get(migration0042.when)).toEqual({ count: 1 });
      expect(sqlite.prepare(`
        SELECT COUNT(*) AS count
        FROM __drizzle_migrations
        WHERE created_at = ?
      `).get(migration0043.when)).toEqual({ count: 0 });
      ...
      expect(sqlite.prepare(`
        ...
        FROM __drizzle_migrations
        WHERE created_at = ?
      `).get(migration0043.when)).toEqual({ count: 1 });
      expect(sqlite.pragma("foreign_key_check")).toEqual([]);
      expect(sqlite.pragma("integrity_check", { simple: true })).toBe("ok");
    } finally {
      sqlite.close();
