...
export const preferences = sqliteTable("preferences", {
    ...
    languagePreference: text("language_preference", { enum: preferenceLanguageCodes })
        .notNull()
        .default("any"),
    watchHistoryOnly: integer("watch_history_only", { mode: "boolean" }).notNull().default(false),
    ...
    historyHideExisting: integer("history_hide_existing", { mode: "boolean" })
        .notNull()
        .default(false),
    historyHideLiked: integer("history_hide_liked", { mode: "boolean" }).notNull().default(false),
    historyHideDisliked: integer("history_hide_disliked", { mode: "boolean" })
        .notNull()
        .default(false),
    historyHideHidden: integer("history_hide_hidden", { mode: "boolean" }).notNull().default(true),
    updatedAt: integer("updated_at", { mode: "timestamp_ms" })
        .notNull()
        .default(sql`(unixepoch() * 1000)`),
});
...
export const serviceConnections = sqliteTable(
    "service_connections",
    {
        ...
        status: text("status", { enum: serviceConnectionStatuses }).notNull().default("configured"),
...
export const downloadQueueItemStatuses = [
    "queued",
    "downloading",
    "paused",
    "completed",
...
export const downloadImportRunStatuses = [
    "pending",
    "running",
    "succeeded",
    "failed",
...
export const engineDownloadFailureKinds = [
    "content",
    "infrastructure",
    "cancelled",
    "unknown",
...
export const mediaLibraryPaths = sqliteTable(
    "media_library_paths",
    {
        ...
        status: text("status", { enum: mediaLibraryPathStatuses }).notNull().default("active"),
...
export const mediaTitles = sqliteTable(
    "media_titles",
    {
        ...
        status: text("status", { enum: mediaTitleStatuses }).notNull().default("missing"),
    ...
    (table) => [
        ...
        index("media_titles_user_media_sort_idx").on(
            table.userId,
            table.mediaType,
            table.sortTitle,
            table.id,
...
export const indexers = sqliteTable(
    "indexers",
    {
        ...
        status: text("status", { enum: indexerConnectionStatuses }).notNull().default("configured"),
        statusMessage: text("status_message"),
...
export const indexerSearchRuns = sqliteTable(
    "indexer_search_runs",
    {
        ...
        status: text("status", { enum: indexerSearchRunStatuses }).notNull().default("pending"),
...
export const indexerSearchResults = sqliteTable(
    ...
    (table) => [
        uniqueIndex("indexer_search_results_run_guid_unique").on(
            table.searchRunId,
            table.indexerGuid,
        ),
...
export const downloadClients = sqliteTable(
    "download_clients",
    {
        ...
        displayName: text("display_name").notNull(),
        status: text("status", { enum: downloadClientStatuses }).notNull().default("configured"),
        statusMessage: text("status_message"),
        isDefault: integer("is_default", { mode: "boolean" }).notNull().default(false),
        lastSyncedAt: integer("last_synced_at", { mode: "timestamp_ms" }),
        createdAt: integer("created_at", { mode: "timestamp_ms" })
...
export const downloadFulfillments = sqliteTable(
    "download_fulfillments",
    {
        ...
        strategy: text("strategy", { enum: downloadFulfillmentStrategies })
            .notNull()
            .default("season_pack"),
        status: text("status", { enum: downloadFulfillmentStatuses }).notNull().default("active"),
...
export const downloadRequests = sqliteTable(
    ...
    (table) => [
        ...
        index("download_requests_target_path_status_idx").on(
            table.targetLibraryPathId,
            table.status,
        ),
        index("download_requests_cancellation_pending_idx")
            .on(table.userId, table.cancellationRequestedAt)
...
export const watchHistorySources = sqliteTable(
    ...
    (table) => [
        uniqueIndex("watch_history_sources_user_type_unique").on(table.userId, table.sourceType),
    ],
);
...
export const watchHistorySyncRuns = sqliteTable(
    "watch_history_sync_runs",
    {
        ...
        userId: text("user_id")
            ...
            .references(() => users.id, { onDelete: "cascade" }),
        mediaType: text("media_type", { enum: recommendationMediaTypes }).notNull(),
        status: text("status", { enum: watchHistorySyncStatuses }).notNull().default("pending"),
    ...
    (table) => [
        index("watch_history_sync_runs_user_created_idx").on(table.userId, table.createdAt),
    ],
);
...
export const recommendationRuns = sqliteTable(
    "recommendation_runs",
    {
        id: text("id").primaryKey(),
        userId: text("user_id")
            ...
            .references(() => users.id, { onDelete: "cascade" }),
        mediaType: text("media_type", { enum: recommendationMediaTypes }).notNull(),
        status: text("status", { enum: recommendationRunStatuses }).notNull().default("pending"),
...
export const recommendationRunMetrics = sqliteTable(
    ...
    (table) => [
        index("recommendation_run_metrics_user_created_idx").on(table.userId, table.createdAt),
    ],
);

export const recommendationItemTimelineEvents = sqliteTable(
    "recommendation_item_timeline_events",
    {
        id: text("id").primaryKey(),
        userId: text("user_id")
            .notNull()
...
export const recommendationFeedback = sqliteTable(
    ...
    (table) => [
        uniqueIndex("recommendation_feedback_user_item_unique").on(table.userId, table.itemId),
    ],
);
...
export const recommendationItemStates = sqliteTable(
    ...
    (table) => [
        uniqueIndex("recommendation_item_states_user_item_unique").on(table.userId, table.itemId),
    ],
);
...
export const notificationChannels = sqliteTable(
    ...
    (table) => [
        uniqueIndex("notification_channels_user_name_unique").on(table.userId, table.displayName),
    ],
);
