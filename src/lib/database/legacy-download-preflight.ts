...
function isLegacyCandidate(row: CandidateRow) {
    ...
    if (effectiveClientType === "nooklet") {
        return false;
    }

    if (effectiveClientType === "sabnzbd") {
        return true;
    }
...
function requestCandidates(sqlite: Database.Database, additionalRequestIds: readonly string[]) {
    const hasCancellation = columnExists(sqlite, "download_requests", "cancellation_requested_at");
    const hasEngineDownloads = tableExists(sqlite, "engine_downloads");
    const cancellationSelection = hasCancellation ? "requests.cancellation_requested_at" : "NULL";
...
function activeImportRequestIds(sqlite: Database.Database) {
    if (!tableExists(sqlite, "download_import_runs")) {
...
export function assertNoActiveLegacyDownloadWork(sqlite: Database.Database) {
    ...
    const candidates = requestCandidates(sqlite, [
        ...new Set([...importRequestIds, ...cancelledPlanRequestIds, ...queuedImportIds]),
    ]);
    ...
    for (const [requestId, rows] of byRequestId) {
        if (!rows.some(isLegacyCandidate)) {
            continue;
        }
