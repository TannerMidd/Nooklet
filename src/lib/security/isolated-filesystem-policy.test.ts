...
describe("resolveApprovedMediaDirectoryIsolated", () => {
  it("returns the canonical path for a directory inside an approved root", async () => {
    ...
    await expect(
      resolveApprovedMediaDirectoryIsolated(candidate, [root]),
    ).resolves.toBe(fs.realpathSync.native(candidate));
  });
  ...
  it("rejects a directory outside the approved roots", async () => {
    ...
    await expect(
      resolveApprovedMediaDirectoryIsolated(outside, [root]),
  ...
  it("abandons a wedged validator without waiting for the child to exit", async () => {
    ...
    ).rejects.toEqual(expect.objectContaining<Partial<IsolatedFilesystemPolicyError>>({
      code: "timeout",
    }));
