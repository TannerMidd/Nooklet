...
describe("safeFetch host classification", () => {
    ...
    it.each(["192.0.0.1", "192.0.2.1", "192.88.99.1", "198.18.0.1", "198.51.100.1", "203.0.113.1"])(
        "rejects reserved IPv4 range %s",
        async (address) => {
            await expect(
...
describe("safeFetch private host override", () => {
    it("allows only the exact private host configured in the environment allowlist", async () => {
        ...
        try {
            ...
            await expect(safeFetch("http://192.168.1.11/")).rejects.toBeInstanceOf(
                SsrfBlockedError,
            );
            await expect(
