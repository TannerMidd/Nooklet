...
function classifyIPv4(ip: string): AddressClassification {
  ...
  if (a === 192 && b === 0 && (parts[2] === 0 || parts[2] === 2)) return "blocked";
  if (a === 192 && b === 88 && parts[2] === 99) return "blocked";
  if (a === 198 && (b === 18 || b === 19)) return "blocked";
  if (a === 198 && b === 51 && parts[2] === 100) return "blocked";
  if (a === 203 && b === 0 && parts[2] === 113) return "blocked";
...
function classifyIPv6(ip: string): AddressClassification {
  ...
  if (/^fe[c-f][0-9a-f]?:/.test(lower)) return "blocked";
  if (/^(?:64:ff9b(?::1)?|100):/.test(lower)) return "blocked";
  if (/^2001:(?:0*:|0*2:|0*10:|0*20:|0*db8:)/.test(lower)) return "blocked";
  if (/^2002:/.test(lower)) return "blocked";
  if (!/^[23][0-9a-f]{3}:/.test(lower)) return "blocked";
...
export async function assertOutboundHostAllowed(
  ...
  if (literalKind) {
    ...
    const timeout = new Promise<never>((_resolve, reject) => {
      timer = setTimeout(() => {
        reject(new SafeFetchAbortError(
          "timeout",
    ...
    const canceled = new Promise<never>((_resolve, reject) => {
      handleAbort = () => reject(new SafeFetchAbortError(
        "canceled",
        "The request was canceled before the host could be resolved.",
      ));
...
export function createPinnedLookup(addresses: readonly ResolvedAddress[]): LookupFunction {
  ...
  return (_hostname, options, callback) => {
    ...
    if (eligible.length === 0) {
      const error = new Error("No prevalidated address matches the requested family.") as NodeJS.ErrnoException;
...
export async function safeFetch(
  ...
  const allowPrivate = options.allowPrivateHosts ?? (
