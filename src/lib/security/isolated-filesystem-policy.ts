...
export function resolveApprovedMediaDirectoryIsolated(
  ...
  const effectiveRoots = configuredRoots.length === 0 && env.NODE_ENV === "test"
  ...
  return new Promise((resolve, reject) => {
    ...
    const deadline = setTimeout(() => {
      finish(() => {
        abandonChild();
        reject(new IsolatedFilesystemPolicyError(
    ...
    child.once("error", () => {
      finish(() => reject(new IsolatedFilesystemPolicyError(
