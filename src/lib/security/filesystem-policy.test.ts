...
describe("filesystem media-root policy", () => {
  ...
  it("does not allow the filesystem root as the configured trust boundary", () => {
    expect(() => resolveApprovedMediaDirectory(libraryRoot, [path.parse(libraryRoot).root])).toThrow(
      /filesystem root/i,
    );
  });
  ...
  it("rejects directories and missing files from deletion", () => {
    expect(() => resolveApprovedMediaFile(libraryRoot, libraryRoot)).toThrow(/regular media files/i);
    expect(() => resolveApprovedMediaFile(path.join(libraryRoot, "missing.mkv"), libraryRoot)).toThrow(
      /no longer exists/i,
    );
  });
});
