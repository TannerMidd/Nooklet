...
import { AlertCircle, ArrowRight, CheckCircle2, Compass, Film, Library, Search, Sparkles, Tv } from "lucide-react";
...
export default async function WorkspaceHomePage() {
  ...
  const setupAttention = readiness.evaluation.capabilities.filter((entry) => entry.status === "needs-attention");
  ...
  return (
    <div className="nk-enter space-y-8">
        ...
        <section className="rounded-2xl border border-accent/35 bg-accent/10 p-5 sm:flex sm:items-center sm:justify-between sm:gap-5">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-muted">
              {readiness.evaluation.completedCoreChecks} of {readiness.evaluation.totalCoreChecks} first-request checks are ready.
              {setupAttention.length > 0 ? ` Next: ${setupAttention.slice(0, 2).map((entry) => entry.title).join(" and ")}.` : ""}
            </p>
            <div className="mt-3 h-2 max-w-xl overflow-hidden rounded-full bg-black/20" role="progressbar" aria-label="Setup progress" aria-valuemin={0} aria-valuemax={100} aria-valuenow={readiness.evaluation.progressPercent}>
              <div className="h-full rounded-full bg-accent" style={{ width: `${readiness.evaluation.progressPercent}%` }} />
            </div>
          </div>
          <Link
            href="/setup"
      ...
      <section aria-labelledby="home-actions-title">
        <h2 id="home-actions-title" className="sr-only">Start something</h2>
      ...
      <Panel
        ...
        actions={(
          <Link href="/setup" className="text-sm font-semibold text-accent hover:text-accent-strong">
            View setup details
          </Link>
        )}
      >
            ...
            <li key={capability.id} className="rounded-xl border border-cream/[0.08] bg-cream/[0.03] p-3.5">
              <div className="flex items-center justify-between gap-2">
                ...
                <Badge variant={capability.status === "ready" ? "accent-cool" : capability.status === "needs-attention" ? "wine" : "neutral"}>
                  {capability.status === "ready" ? "Ready" : capability.status === "needs-attention" ? "Attention" : "Optional"}
                </Badge>
              </div>
              <p className="mt-2 text-xs leading-5 text-muted">{capability.summary}</p>
            </li>
          ))}
        </ul>
        {readiness.evaluation.setupComplete ? (
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Panel
          ...
          actions={(
            <Link href="/in-progress?view=attention" className="text-sm font-semibold text-accent hover:text-accent-strong">
              Open Activity
            </Link>
          )}
        >
              ...
              {needsAttention.slice(0, 4).map((entry) => (
                <li key={entry.id} className="flex gap-3 py-3 first:pt-0 last:pb-0">
                  ...
                  <div className="min-w-0">
                    <p className="truncate text-sm font-semibold text-foreground">{entry.requestedTitle}</p>
                    <p className="mt-0.5 text-sm leading-5 text-muted">{friendlyFailure(entry.statusMessage)}</p>
                  </div>
                </li>
              ))}
            </ul>
        ...
        <Panel
          ...
          actions={(
            <Link href="/library" className="text-sm font-semibold text-accent hover:text-accent-strong">
              Open Library
            </Link>
          )}
        >
              ...
              {completed.slice(0, 5).map((entry) => (
                <li key={entry.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-semibold text-foreground">{entry.requestedTitle}</p>
                    <p className="text-xs capitalize text-muted">{entry.mediaType === "tv" ? "TV" : "Movie"} · Available</p>
                  </div>
                </li>
              ))}
            </ul>
