...
export type LibraryBrowseSearchParamsInput = Record<
  string,
...
const libraryBrowseSearchParamsSchema = z.object({
  ...
  details: z
    .preprocess(optionalSearchParamValue, z.string().uuid().optional())
    .catch(undefined),
  status: z
    .preprocess(
  ...
  view: z.preprocess(
    optionalSearchParamValue,
    z.enum(["list", "grid"]).catch("list"),
  ),
});

export function parseLibraryBrowseSearchParams(
  input: LibraryBrowseSearchParamsInput = {},
) {
