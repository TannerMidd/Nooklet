...
function usedPercent(entry: LibraryDriveEntry) {
  if (entry.totalSpaceBytes === null || entry.freeSpaceBytes === null || entry.totalSpaceBytes <= 0) {
    return null;
  }
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 space-y-0.5">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {entry.isDownloadDefault ? <Badge variant="highlight">Default</Badge> : null}
            {!entry.live ? <Badge>Last known</Badge> : null}
          </div>
          <p className="truncate text-xs text-muted">{entry.path} / {entry.libraryName}</p>
        </div>
...
export function LibraryDrivesPanel({ entries }: { entries: LibraryDriveEntry[] }) {
  if (entries.length === 0) {
    return <EmptyState message="Attach a library folder to see drive space and set download defaults." />;
  }

  return (
    <div className="space-y-3">
      <p className="text-sm leading-6 text-muted">
        Downloads without an explicit destination go to the default folder for
        their media type. Marked folders are preselected in every request form.
      </p>
