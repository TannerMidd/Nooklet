...
function RemoveButton({ pending, queued, onClick }: { pending: boolean; queued: boolean; onClick: () => void }) {
  return (
    <Button type="button" variant="danger" size="sm" disabled={pending || queued} onClick={onClick}>
  ...
  return (
    <>
      <form ref={formRef} action={formAction} className="flex flex-col gap-2">
        ...
        <label className="flex min-h-11 items-center gap-2 text-xs text-muted">
          <input
            ...
            onChange={(event) => {
              ...
              if (event.target.checked) setRetireActiveWork(false);
            }}
        ...
        <div className={state.action === "open_activity" ? "rounded-lg border border-accent-wine/30 bg-accent-wine/10 px-3 py-2" : undefined}>
          {state.action === "open_activity" ? (
            <p className="mb-1 text-xs font-semibold text-foreground">Safe removal can finish this automatically:</p>
          ) : null}
          <label className="flex min-h-11 items-start gap-2 text-xs leading-5 text-muted">
            ...
            />
            Stop active season plans and downloads first, then remove this title. Imported media files stay on disk.
          </label>
        </div>
        <RemoveButton
          pending={pending}
        ...
        {state.message ? (
          ...
          >
            ...
            {state.action === "queued_removal" ? (
              <>
                {" "}
                <Link href="/health" className="font-semibold text-accent underline underline-offset-2">
                  View background job
                </Link>
              </>
            ) : null}
      ...
      <AlertDialog
        ...
        description={deleteFiles ? (
          ...
          <>
            Nooklet will persist this request, stop active season recovery and downloader jobs, verify their cleanup, and then remove the title from the library. Imported media and other library files will stay on disk. Incomplete downloader files owned by the cancelled work may be removed.
          </>
        ) : (
