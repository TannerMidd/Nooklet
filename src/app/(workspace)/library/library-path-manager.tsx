    ...
    return (
        <li className="border-t border-cream/[0.08] py-2 text-sm first:border-t-0">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-muted">
                <span>
                    {libraryPath.fileCount} file{libraryPath.fileCount === 1 ? "" : "s"}
                </span>
                {libraryPath.lastScannedAt ? (
                    <span>scanned {libraryPath.lastScannedAt.toLocaleString()}</span>
                ) : null}
