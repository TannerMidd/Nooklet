...
const titleStatusPresentation = {
    ...
    MediaTitleStatus,
    { label: string; tone: "ok" | "active" | "error" | "neutral" }
>;
...
function buildLibraryPageHref(
    ...
    if (browse.status) {
        params.set("status", browse.status);
    }

    if (typeof browse.monitored === "boolean") {
        params.set("monitored", browse.monitored ? "yes" : "no");
    }

    if (browse.libraryId) {
        params.set("library", browse.libraryId);
    }

    if (browse.sort && browse.sort !== "title") {
            ...
            >
                ...
                <div className="min-w-0">
                    ...
                    <p className="truncate text-xs text-muted md:hidden">
                        ...
                        {getMediaQualityProfileLabel(title.qualityProfile)} / {fileLabel}
                    </p>
                </div>
                <p className="hidden truncate text-muted md:block">
                    {title.libraryName ?? "Unassigned"}
                </p>
            ...
            >
                ...
                <div className="relative aspect-[2/3] overflow-hidden bg-panel">
                    {title.posterUrl ? (
                        ...
                        <span className="flex h-full items-center justify-center px-4 text-center text-sm text-muted">
                            No artwork
                        </span>
                    )}
...
export async function LibraryTitlePage({
    ...
    const [library, lastScannedAt, libraryOverview] = await Promise.all([
        listMediaLibraryTitles(session.user.id, mediaType, {
            query,
            page,
            status,
            monitored,
    ...
    const targetPathOptions = hasSelectedTitle
        ? (await listMediaLibraryPathOptions(session.user.id)).filter(
              (option) =>
                  option.mediaType === mediaType &&
                  (selectedLibraryId ? option.libraryId === selectedLibraryId : true),
          )
        : [];
    const currentLibraryPathId =
        hasSelectedTitle && detailsTitleId
            ? await getMediaTitleCurrentLibraryPathId({
        ...
        <div className="nk-enter space-y-7">
            <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div className="min-w-0">
                    ...
                    <p className="mt-2 text-[13px] text-muted">
                        {titleCountLabel(mediaType, library.totals.titles)} / {library.totals.files}{" "}
                        files / {library.totals.available} available / {library.totals.requested}{" "}
                        requested / {library.totals.missing} missing / {library.totals.monitored}{" "}
                        monitored
                    </p>
                </div>
            ...
            >
                <label className="flex min-h-11 items-center gap-2 rounded-lg border border-cream/[0.10] bg-cream/[0.04] px-3">
                    ...
                    <input
                        name="q"
                        defaultValue={query ?? ""}
                        placeholder={mediaType === "tv" ? "Series title" : "Movie title"}
                ...
                <select
                    ...
                    name="status"
                    defaultValue={status ?? ""}
                    className="min-h-11 rounded-lg border border-cream/[0.10] bg-cream/[0.04] px-3 text-sm text-foreground"
                >
                ...
                <label className="sr-only" htmlFor="library-monitoring">
                    Monitoring
                </label>
                <select
                    id="library-monitoring"
                    name="monitored"
                    defaultValue={typeof monitored === "boolean" ? (monitored ? "yes" : "no") : ""}
                    className="min-h-11 rounded-lg border border-cream/[0.10] bg-cream/[0.04] px-3 text-sm text-foreground"
                >
                ...
                <label className="sr-only" htmlFor="library-destination">
                    Library
                </label>
                <select
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
                <SegmentedLinks
                    ...
                    options={[
                        {
                            href: buildLibraryPageHref(mediaType, query, 1, null, {
                                ...browse,
                                view: "list",
                            }),
                            label: "List",
                ...
                {hasFilters ? (
                    <Link
                        href={mediaType === "tv" ? "/library/tv" : "/library/movies"}
