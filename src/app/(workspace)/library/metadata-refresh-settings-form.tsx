...
import { initialMetadataRefreshScheduleActionState } from "@/app/(workspace)/library/action-state";
...
export function MetadataRefreshSettingsForm({ settings }: { settings: MetadataRefreshSettings }) {
    ...
    return (
        <form action={formAction} className="space-y-4">
                ...
                <label className="flex items-start gap-2.5 rounded-lg border border-cream/[0.08] bg-cream/[0.03] px-3 py-2.5">
                    ...
                    <span className="space-y-1">
                        ...
                        <span className="block text-sm leading-6 text-muted">
                            Sync monitored series with TMDB so new seasons and episodes are detected
                            automatically.
                        </span>
                    </span>
                </label>
            ...
            {state.message ? <StatusMessage status={state.status} message={state.message} /> : null}
