    ...
    try {
        ...
        if (result.queued) {
            return {
                status: "success",
                message:
                    result.reason === "episode_fallback"
                        ? (result.message ??
                          "No usable season pack remained, so Nooklet queued missing episodes individually.")
        ...
        if (result.reason === "search_failed") {
            return {
                ...
                message:
                    result.message ??
                    "The indexer search failed. Check your indexer, then try again.",
            };
        }
        ...
        return {
            status: "error",
            message:
                result.message ?? "A release was found, but the downloader could not queue it.",
        };
    } catch (error) {
        if (error instanceof RetryDownloadRequestWorkflowError) {
            return { status: "error", message: error.message };
