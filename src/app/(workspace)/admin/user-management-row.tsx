...
export function UserManagementRow({ currentAdminUserId, user }: UserManagementRowProps) {
    ...
    function confirmAction() {
        if (confirmationKind === "role") {
            roleFormRef.current?.requestSubmit();
        }

        if (confirmationKind === "status") {
            statusFormRef.current?.requestSubmit();
        }

        if (confirmationKind === "password") {
            ...
            >
                <div className="space-y-6 p-5">
                    <div className="rounded-xl border border-cream/[0.08] bg-cream/[0.03] p-4 text-sm leading-6">
                        ...
                        <p className="mt-1 text-muted">
                            Account is {user.isDisabled ? "disabled" : "active"}.
                        </p>
                    </div>
                    ...
                    <form
                        ref={roleFormRef}
                        action={roleAction}
                        className="space-y-3 rounded-xl border border-cream/[0.08] p-4"
                    >
                        <input type="hidden" name="userId" value={user.id} />
                        <FormField
                            label="Role"
                            description="Administrators can change all shared server configuration."
                        >
                            {(controlProps) => (
                                <select
                                    {...controlProps}
                    ...
                    >
                        ...
                        <div>
                            ...
                            <p className="mt-1 text-sm leading-6 text-muted">
                                Resetting takes effect immediately and signs older sessions out.
                            </p>
                        </div>
                        <FormField
                            label="New password"
                            ...
                            description="At least 12 characters with uppercase, lowercase, and a number."
                        >
                            {(controlProps) => (
                                <Input
                                    {...controlProps}
                        ...
                        <Button
                            ...
                            variant="secondary"
                            disabled={passwordPending}
                            onClick={() => setConfirmationKind("password")}
                        >
