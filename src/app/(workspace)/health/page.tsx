...
function capabilityBadge(capability: ReadinessCapability) {
    if (capability.status === "ready") {
        return { label: "Ready", variant: "accent-cool" as const };
    }

    if (capability.status === "needs-attention") {
        return { label: "Needs attention", variant: "wine" as const };
    }
...
function jobLabel(jobType: string) {
    switch (jobType) {
        ...
        case "metadata-refresh":
            return "TV metadata refresh";
        case "missing-content-search":
            return "Missing-content search";
        ...
        <li className="rounded-xl border border-accent-wine/25 bg-accent-wine/[0.07] p-4">
            ...
            {capability.id === "worker" ? (
                <p className="mt-3 rounded-lg border border-cream/[0.08] bg-black/10 px-3 py-2 text-xs leading-5 text-muted">
                    Restart the Nooklet app or container, then reload this page. If the worker
                    responds but stays degraded, expand its technical error below.
                </p>
            ) : (
                <Link
...
export default async function HealthPage() {
    ...
    const attentionCapabilities = evaluation.capabilities.filter(
        (entry) => entry.status === "needs-attention",
    );
    const optionalCapabilities = evaluation.capabilities.filter(
        (entry) => entry.status === "optional",
    );
    const configuredServices = services.filter(
        (connection) => connection.status !== "disconnected",
    );
    const unconfiguredServices = services.filter(
        (connection) => connection.status === "disconnected",
    );
    ...
    return (
        <div className="nk-enter space-y-7">
            ...
            {attentionCapabilities.length === 0 ? (
                <div className="flex items-start gap-3 rounded-2xl border border-accent-cool/25 bg-accent-cool/10 p-4 text-sm leading-6 text-foreground">
                    ...
                    <div>
                        ...
                        <p className="text-muted">
                            At least one complete request path is ready. Optional integrations do
                            not count as system failures.
                        </p>
                    </div>
                </div>
            ) : (
                <Panel
                    title="Action needed"
                    description="These issues affect a complete user capability and include the shortest path to recovery."
                >
                    <ul className="grid gap-3 lg:grid-cols-2">
                        {attentionCapabilities.map((capability) => (
                            <ActionCapability key={capability.id} capability={capability} />
                        ))}
                    </ul>
                </Panel>
            )}
            ...
            <Panel
                ...
                description="Runs recommendations, schedules, download processing, and imports."
            >
                <article
                    className={`rounded-xl border p-4 text-sm leading-6 ${statusTone(workerHealthy ? "verified" : worker.responsive ? "running" : "failed")}`}
                >
                    <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                            ...
                            <p className="mt-1 text-muted">
                                {worker.worker.runningMaintenance
                                    ? "A maintenance pass is running now."
                                    : `Last successful pass: ${formatDate(worker.worker.lastSuccessAt)}.`}
                            </p>
                        </div>
                        <Badge variant={workerHealthy ? "accent-cool" : "wine"}>
                            {workerLabel}
                        </Badge>
                    </div>
                    <div className="mt-3 flex flex-wrap gap-x-5 gap-y-1 text-xs text-muted">
                        ...
                        <span>
                            {activeJobs.length} enabled or running{" "}
                ...
                <article
                    className={`mt-3 rounded-xl border p-4 text-sm leading-6 ${statusTone(downloadEngine.status === "degraded" ? "failed" : downloadEngine.status === "ok" ? "running" : "verified")}`}
                >
                    <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                            <p className="font-semibold text-foreground">
                    ...
                    <div className="mt-3 flex flex-wrap gap-x-5 gap-y-1 text-xs text-muted">
                        <span>
                            Last persisted progress {formatDate(downloadEngine.lastProgressAt)}
                        </span>
                        <span>{downloadEngine.stalledCount} stalled</span>
                        <span>
                            {downloadEngine.failedCount} unresolved infrastructure{" "}
                            {downloadEngine.failedCount === 1 ? "failure" : "failures"}
                        </span>
                    </div>
                    {downloadEngine.issues.length > 0 ? (
                        <details className="mt-3 rounded-lg border border-cream/[0.08] bg-black/10 px-3 py-2">
                            <summary className="cursor-pointer font-semibold text-foreground">
                                Download engine diagnostics
                            </summary>
                ...
                {unconfiguredServices.length > 0 ? (
                    <details className="mt-4 rounded-xl border border-cream/[0.08] bg-cream/[0.02] px-4 py-3">
                        ...
                        <p className="mt-2 text-xs leading-5 text-muted">
                            These are available integrations, not health failures. Setup Center
                            identifies any one that is required for your chosen request path.
                        </p>
                        <ul className="mt-3 flex flex-wrap gap-2">
                            {unconfiguredServices.map((connection) => (
                                <li key={connection.serviceType}>
                                    <Badge>{connection.displayName}</Badge>
                                </li>
                            ))}
                        </ul>
                    </details>
            ...
            <div className="grid gap-6 xl:grid-cols-2">
                ...
                >
                        ...
                        <ul className="space-y-3">
                            {optionalCapabilities.map((capability) => (
                                ...
                                >
                                    ...
                                    <Link
                                        href={capability.remediationHref}
                                        className="mt-2 inline-flex text-sm font-semibold text-accent hover:text-accent-strong"
                                    >
                                        {capability.remediationLabel}
                                    </Link>
                                </li>
                    ...
                    <p className="mt-3 text-xs text-muted">
                        {watchHistory.totalCount} watched titles across{" "}
                        {watchHistory.sources.length} sources.
                    </p>
                </Panel>

                <Panel
                    title="Background jobs"
                    description="Failures are surfaced first; routine schedules stay collapsed."
                >
                    {failedJobs.length > 0 ? (
                        <ul className="space-y-3">
                            {failedJobs.map((job) => {
                                ...
                                return (
                                    <li
                                        key={job.id}
                                        className="rounded-xl border border-accent-wine/25 bg-accent-wine/[0.07] p-3.5 text-sm"
                                    >
                                        <div className="flex items-start gap-2">
                                            ...
                                            <div>
                                                ...
                                                <Link
                                                    href={recovery.href}
                                                    className="mt-2 inline-flex min-h-11 items-center font-semibold text-accent hover:text-accent-strong"
                                                >
                                                    {recovery.label}
                                                    <ChevronRight
                                                        aria-hidden="true"
                    ...
                    {jobs.length > 0 ? (
                        <details className="mt-4 rounded-xl border border-cream/[0.08] bg-cream/[0.02] px-4 py-3">
                            <summary className="cursor-pointer text-sm font-semibold text-foreground">
                                All job details ({jobs.length})
                            </summary>
                            <ul className="mt-3 space-y-3">
                                {jobs.map((job) => (
                                    <li
                                        key={job.id}
                                        className="rounded-lg border border-cream/[0.08] bg-cream/[0.03] p-3 text-xs leading-5 text-muted"
                                    >
                                        ...
                                        <p className="mt-1">
                                            Next {formatDate(job.nextRunAt)} · Last completed{" "}
                                            {formatDate(job.lastCompletedAt)}
                                        </p>
                                        <p className="mt-1 font-mono">
                                            {job.targetType}: {job.targetKey}
                                        </p>
                                    </li>
                                ))}
                            </ul>
