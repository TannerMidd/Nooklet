...
export default async function AnalyticsPage() {
    ...
    const funnelShare = (value: number) =>
        generated > 0 ? Math.round((value / generated) * 100) : 0;
    const funnelRows = [
        {
            label: "Generated",
        ...
        <div className="nk-enter space-y-9">
                        ...
                        {funnelRows.map((row) => (
                            <div key={row.label}>
                                <div className="flex items-baseline justify-between gap-3">
                                    ...
                                    <span
                                        className={`text-[15px] font-semibold tabular-nums ${row.valueClass}`}
                                    >
                                        {row.value}
                                    </span>
                                </div>
                ...
                <section className="space-y-4">
                    ...
                    <div className="space-y-4.5 rounded-2xl border border-cream/[0.08] bg-cream/[0.03] p-5">
                        <div className="space-y-2.5">
                            <TasteBar
                                label="Likes"
                                value={allTaste.likeCount}
                                max={tasteMax}
                                barClass="bg-accent-cool"
                            />
                            <TasteBar
                                label="Added to library"
                                value={allTaste.addedCount}
                                max={tasteMax}
                                barClass="bg-accent"
                            />
                            <TasteBar
                                label="Dislikes"
                                value={allTaste.dislikeCount}
                                max={tasteMax}
                                barClass="bg-accent-wine"
                            />
                            <TasteBar
                                label="Hidden"
                                value={allTaste.hiddenCount}
                                max={tasteMax}
                                barClass="bg-muted"
                            />
                        </div>
                                        ...
                                        <div className="min-w-0">
                                            <p className="text-sm font-semibold text-foreground">
                                                ...
                                                <span className="font-normal text-muted">
                                                    —{" "}
                                                    {run.requestPrompt ||
                                                        "Taste-based automatic request"}
                                                </span>
                                            </p>
                                            <p className="mt-0.5 text-[12.5px] text-muted">
                                                {formatNumber(run.totalTokens)} tokens ·{" "}
                                                {formatDuration(run.durationMs)} ·{" "}
                                                {run.generatedItemCount} saved ·{" "}
                                                {run.generationAttemptCount} attempt
            ...
            <div className="grid gap-6 xl:grid-cols-2">
                <section className="space-y-4">
                    ...
                    <TasteList
                        items={
                            tvTaste.likedItems.length > 0 ? tvTaste.likedItems : tvTaste.addedItems
                        }
                    />
                </section>
                <section className="space-y-4">
                    ...
                    <TasteList
                        items={
                            movieTaste.likedItems.length > 0
                                ? movieTaste.likedItems
                                : movieTaste.addedItems
                        }
                    />
                </section>
            </div>
