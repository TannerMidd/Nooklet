...
export default async function DiscoverPage({ searchParams }: DiscoverPageProps) {
    ...
    const filters: DiscoverFilters = {
        media:
            resolvedSearchParams?.media === "movie" || resolvedSearchParams?.media === "tv"
                ? resolvedSearchParams.media
                : "all",
        yearFrom:
            Number.isInteger(yearFromValue) &&
            yearFromValue >= 1900 &&
            yearFromValue <= new Date().getFullYear() + 2
                ? yearFromValue
                : null,
        rating:
    ...
    const [overview, selectedOverview, libraryOverview, pathOptions, exclusions] =
        await Promise.all([
        ...
        ? overview.rails
              ...
              .map((rail) => ({
                  ...
                  titles: rail.titles.filter((title) => {
                      if (filters.yearFrom && (!title.year || title.year < filters.yearFrom)) {
                          return false;
                      }

                      if (
                          filters.rating &&
                          (!title.voteAverage || title.voteAverage < filters.rating)
                      ) {
                          return false;
                      }
                      ...
                      if (
                          filters.hideOwned &&
                          exclusions.ownedTmdbKeys.has(`${rail.mediaType}-${title.tmdbId}`)
                      ) {
                          return false;
                      }
    ...
    const selectedRail = overview.ok
        ? (filteredRails.find(
              (rail) => railKey(rail.category, rail.mediaType) === selectedRailKey,
          ) ?? null)
        : null;
        ...
        <div className="nk-enter space-y-9">
            <PageHeader
                ...
                actions={
                    selectedRail ? (
                        <Link
                            href={buildDiscoverHref(filters)}
                            className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-cream/[0.14] px-4 py-2 text-sm font-semibold text-foreground hover:bg-cream/[0.05]"
                        >
                        ...
                        <Link
                            href="/search"
                            className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-accent px-5 py-2 text-sm font-semibold text-accent-foreground hover:bg-accent-strong"
                        >
            ...
            >
                {!selectedRail ? (
                    <nav aria-label="Personalized discovery" className="flex flex-wrap gap-2">
                        <Link
                            href="/movies"
                            className="inline-flex min-h-11 items-center rounded-full border border-cream/[0.14] bg-cream/[0.03] px-4 text-sm font-semibold text-foreground hover:bg-cream/[0.07]"
                        >
                            Movie ideas
                        </Link>
                        <Link
            ...
            <form
                action="/discover"
                className="grid gap-3 rounded-2xl border border-cream/[0.08] bg-cream/[0.025] p-4 sm:grid-cols-2 lg:grid-cols-[repeat(3,minmax(140px,1fr))_auto_auto_auto] lg:items-center"
            >
                <label className="space-y-1 text-xs font-semibold text-muted">
                    ...
                    <select
                        ...
                        className="block min-h-11 w-full rounded-lg border border-cream/[0.10] bg-cream/[0.04] px-3 text-sm text-foreground"
                    >
                ...
                <label className="flex min-h-11 items-center gap-2 text-sm font-semibold text-foreground">
                    <input
                        ...
                        name="hideOwned"
                        value="1"
                        defaultChecked={filters.hideOwned}
                ...
                <label className="flex min-h-11 items-center gap-2 text-sm font-semibold text-foreground">
                    <input
                        ...
                        name="hideWatched"
                        value="1"
                        defaultChecked={filters.hideWatched}
                        className="h-5 w-5 accent-accent"
                    />{" "}
                    Hide watched
                </label>
                <div className="flex items-center gap-2">
                    ...
                    {hasFilters ? (
                        <Link
                            href="/discover"
                            className="inline-flex min-h-11 items-center rounded-lg px-3 text-sm font-semibold text-accent"
                        >
                            Clear
                        </Link>
                    ) : null}
                    ...
                    <section key={`${rail.category}-${rail.mediaType}`} className="space-y-4">
                        <div className="flex items-end justify-between gap-4">
                            ...
                            {!selectedRail ? (
                                <Link
                                    href={buildDiscoverHref(filters, {
                                        rail: railKey(rail.category, rail.mediaType),
                                    })}
                        ...
                        {rail.titles.length === 0 ? (
                            <p className="text-sm leading-6 text-muted">
                                TMDB returned no titles for this rail right now.
                            </p>
                        ) : (
                            <ul
                                className={
                                            ...
                                            <div className="relative aspect-[2/3] w-full overflow-hidden rounded-lg border border-cream/[0.10] bg-panel shadow-[0_18px_34px_-24px_rgba(0,0,0,0.8)]">
                                                {title.posterUrl ? (
                                                    <Image
                                                        ...
                                                        sizes={
                                                            selectedRail
                                                                ? "(min-width: 1280px) 12rem, 30vw"
                                                                : "(min-width: 640px) 11rem, 48vw"
                                                        }
            ...
            {selectedOverview && selectedOverview.ok ? (
                <DiscoverTitleOverviewDialog
                    ...
                    closeHref={closeHref}
                    returnTo={buildOverviewHref(
                        selectedOverview.details.mediaType,
                        selectedOverview.details.tmdbId,
                        filters,
                        selectedRailKey,
                    )}
            ...
            {selectedOverview && !selectedOverview.ok ? (
                <Panel
                    eyebrow={
                        selectedOverview.reason === "tmdb-not-configured"
                            ? "TMDB required"
                            : "Title unavailable"
                    }
