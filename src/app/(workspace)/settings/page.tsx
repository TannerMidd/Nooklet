...
export default async function SettingsOverviewPage() {
  ...
  const attentionCount = evaluation.capabilities.filter((entry) => entry.status === "needs-attention").length;
  ...
  return (
    <div className="nk-enter space-y-7">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="font-semibold text-foreground">
              {evaluation.setupComplete ? "Core setup is ready" : `${attentionCount} ${attentionCount === 1 ? "area needs" : "areas need"} attention`}
            </p>
            <p className="mt-1 text-sm text-muted">{evaluation.progressPercent}% of the first-request path is configured.</p>
          </div>
      ...
      >
        <SettingsCards
          entries={session.user.role === "admin"
            ? [...instanceSettings, ["Users", "Create accounts, change roles, reset passwords, and disable access.", "/admin"] as const]
            : instanceSettings}
        />
      </Panel>
    </div>
