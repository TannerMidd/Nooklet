...
import {
  getIndexerProviderPreset,
  indexerProviderPresets,
} from "./indexer-presets";
...
function hasCategory(indexer: IndexerSettingsView | undefined, mediaType: "movie" | "tv", categoryId: string) {
  return indexer?.categories.some(
...
function customCategoryValues(indexer: IndexerSettingsView | undefined, mediaType: "movie" | "tv") {
  ...
  return indexer?.categories
    .filter((category) => category.mediaType === mediaType && category.categoryId !== standardId)
...
function SaveButtons({ isEditing, disabled = false }: { isEditing: boolean; disabled?: boolean }) {
  ...
  return (
    <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
      <Button type="submit" name="intent" value="test-save" className="w-full sm:w-auto" disabled={pending || disabled}>
      ...
      <Button type="submit" name="intent" value="save" variant="secondary" className="w-full sm:w-auto" disabled={pending || disabled}>
...
function TestButton({ disabled = false }: { disabled?: boolean }) {
  ...
  return (
    <Button type="submit" variant="secondary" className="w-full sm:w-auto" disabled={pending || disabled}>
...
export function IndexerSettingsForm({ indexer }: { indexer?: IndexerSettingsView }) {
  ...
  const inferredPreset = indexerProviderPresets.find(
        ...
        <div className="space-y-3">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 truncate text-sm text-muted" title={indexer.baseUrl}>{indexer.baseUrl}</p>
              <p className="mt-1 text-xs leading-5 text-muted">
                Search order {indexer.priority} · {indexer.categories.length > 0
                  ? Array.from(new Set(indexer.categories.map((category) => category.mediaType === "tv" ? "TV" : "Movies"))).join(" and ")
                  : "No media categories"}
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button type="button" variant="secondary" size="sm" onClick={() => setEditOpen((open) => !open)}>
          ...
          {indexer.statusMessage ? <p className="text-sm leading-6 text-muted">{indexer.statusMessage}</p> : null}
        </div>
      ) : null}

      {legacyTorznab ? (
        <InlineAlert variant="warning">
          Torznab is not supported by this Usenet-only downloader. Switch this indexer to Newznab before saving or testing it.
        </InlineAlert>
      ) : null}

      {editOpen ? (
        <form action={formAction} className="max-w-3xl space-y-5 border-t border-cream/[0.07] pt-4">
          ...
          <label className="block space-y-1.5 text-sm">
            ...
            <p className="text-[13px] leading-5 text-muted">
              {getIndexerProviderPreset(providerPreset).description} Review the URL against the provider account page before saving.
            </p>
          </label>
          ...
          <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_180px]">
            <label className="space-y-1.5 text-sm">
              ...
              <Input name="name" placeholder="My indexer" value={name} onChange={(event) => setName(event.target.value)} required />
            </label>
            <label className="space-y-1.5 text-sm">
              ...
              >
                ...
                {indexer?.protocol === "torznab" ? (
                  <option value="torznab" disabled>Torznab (unsupported)</option>
                ) : null}
              </select>
            </label>
          </div>
          ...
          <label className="block space-y-1.5 text-sm">
            ...
            <Input name="baseUrl" type="url" placeholder="https://indexer.example.com" value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} required />
            <p className="text-[13px] leading-5 text-muted">Use the HTTPS Newznab host from your provider. Nooklet adds the API path below.</p>
          </label>
          ...
          <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_160px]">
            <label className="space-y-1.5 text-sm">
              ...
              <Input
                ...
                placeholder={indexer ? "Leave blank to keep saved key" : "Paste provider API key"}
                required={!indexer}
              />
            </label>
            <label className="space-y-1.5 text-sm">
              ...
              <p className="text-[12px] leading-5 text-muted">Lower numbers run first.</p>
            </label>
          </div>
          ...
          <fieldset className="space-y-2">
            ...
            <p className="text-[13px] leading-5 text-muted">These standard Newznab categories work for most providers.</p>
          </fieldset>
          ...
          <details className="rounded-lg border border-cream/[0.08] bg-cream/[0.02]">
            ...
            <div className="grid gap-4 border-t border-cream/[0.07] p-3.5 md:grid-cols-2">
              <label className="space-y-1.5 text-sm">
                <span className="font-medium text-foreground">Additional movie category IDs</span>
                <Input name="movieCustomCategories" placeholder="2040" defaultValue={customCategoryValues(indexer, "movie")} />
              </label>
              <label className="space-y-1.5 text-sm">
                <span className="font-medium text-foreground">Additional TV category IDs</span>
                <Input name="tvCustomCategories" placeholder="5040" defaultValue={customCategoryValues(indexer, "tv")} />
              </label>
              <label className="space-y-1.5 text-sm md:col-span-2">
                <span className="font-medium text-foreground">API path</span>
                <Input name="apiPath" value={apiPath} onChange={(event) => setApiPath(event.target.value)} required />
                <p className="text-[13px] leading-5 text-muted">Usually <code>/api</code>. Change it only when your provider documents another Newznab path.</p>
              </label>
            </div>
          </details>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <label className="flex min-h-11 items-center gap-3 text-sm text-foreground">
              <input type="checkbox" name="isEnabled" defaultChecked={indexer?.isEnabled ?? true} className="h-4 w-4 accent-accent" />
              <span>Use this indexer in searches</span>
            </label>
            <SaveButtons isEditing={isEditing} disabled={legacyTorznab} />
          </div>
          <p className="text-[13px] leading-5 text-muted">
            Test & save checks these draft values first. If the test fails, the currently saved indexer remains unchanged.
          </p>
        </form>
      ) : null}
      ...
      {indexer ? (
        <>
          ...
          <AlertDialog
            ...
            description={(
              <p>
                Nooklet will remove its saved API key and categories. Existing activity history remains, but future searches will no longer use this indexer.
              </p>
            )}
