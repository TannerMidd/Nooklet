...
function readCategories(formData: FormData, mediaType: RecommendationMediaType) {
    ...
    const customCategories = parseCategoryList(
        formData.get(`${mediaType}CustomCategories`),
        mediaType,
    );
    ...
    return Array.from(
        new Map(
            [...friendlyCategories, ...customCategories, ...legacyCategories].map((category) => [
                category.categoryId,
                category,
            ]),
        ).values(),
...
export async function addIndexerAction(
    ...
    if (!parsed.success) {
        const firstIssue =
            parsed.error.issues[0]?.message ?? "Review the indexer settings and try again.";
...
export async function removeIndexerAction(
    ...
    try {
        ...
        return {
            status: "error",
            message:
                error instanceof RemoveIndexerCommandError
                    ? error.message
