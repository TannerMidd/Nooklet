        ...
        <div className="nk-enter space-y-7">
            ...
            {session.user.role !== "admin" ? (
                <p className="rounded-xl border border-accent/25 bg-accent/10 p-4 text-sm leading-6 text-foreground">
                    Library automation is shared across the Nooklet instance and can only be changed
                    by an administrator. Your personal history sync schedules remain editable below.
                </p>
            ) : (
                <>
                    <Panel
                        eyebrow="Library"
                        title="Library scan"
                        description="Discover new and changed files in attached movie and TV folders."
                    >
                    ...
                    <Panel
                        eyebrow="Downloads"
                        title="Missing-content search"
                        description="Look for monitored titles and episodes that are still unavailable."
                    >
                    ...
                    <Panel
                        eyebrow="Metadata"
                        title="Series metadata refresh"
                        description="Check monitored series for new seasons and episodes from TMDB."
                    >
            ...
            >
                {historyOverview.sources.length > 0 ? (
                    <div className="grid gap-4 lg:grid-cols-2">
                        {historyOverview.sources.map((source) => {
                            ...
                            return (
                                <div
                                    key={source.id}
                                    className="rounded-xl border border-cream/[0.08] bg-cream/[0.025] p-4"
                                >
                                    ...
                                    <WatchHistoryScheduleForm
                                        ...
                                        defaultIntervalHours={Math.max(
                                            Math.round((schedule?.scheduleMinutes ?? 720) / 60),
                                            1,
                                        )}
                    ...
                    <div className="space-y-3 text-sm leading-6 text-muted">
                        <p>
                            Run a first sync under History sources. Nooklet will then offer a
                            schedule here using those saved source choices.
                        </p>
                        <Link
                            href="/settings/history"
                            className="inline-flex min-h-11 items-center rounded-lg border border-cream/[0.14] px-4 font-semibold text-foreground"
                        >
                            Set up a history source
                        </Link>
                    </div>
                )}
            </Panel>
