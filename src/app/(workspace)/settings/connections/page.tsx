...
const connectionGroups = [
  {
    ...
    description: "TMDB powers discovery and artwork. TVDB can add another source of episode and series metadata.",
    connections: [
  ...
  },
  {
    ...
    description: "AI recommendations are optional. Nooklet's library and manual search continue to work without them.",
    connections: [
      { serviceType: "ai-provider", requirement: "Required for AI picks" },
    ],
  },
  {
    ...
    <div className="nk-enter space-y-7">
      ...
      {session.user.role !== "admin" ? (
        <p className="rounded-xl border border-accent/25 bg-accent/[0.08] px-4 py-3 text-sm leading-6 text-foreground">
          Instance services are shared by the administrator and power requests for every user. You can connect your own Trakt account for personal watch history.
        </p>
      ) : null}

      {connectionGroups.map((group) => {
        const cards = group.connections.flatMap((connection) => {
          const summary = summaries.find((item) => item.serviceType === connection.serviceType);
            ...
            <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">
              {cards.map(({ summary, requirement }) => (
                <ConnectionCard
                  ...
                  summary={summary}
                  canManage={session.user.role === "admin" || summary.serviceType === "trakt"}
