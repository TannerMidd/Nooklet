...
const usenetFormSchema = z
    .object({
        ...
        usenetConnections: z.coerce
            .number()
            .int()
            .min(1, "Use at least one connection.")
            .max(20, "Nooklet supports up to 20 connections."),
    ...
    .superRefine((value, context) => {
        ...
        if (
            (value.usenetUsername && !value.usenetPassword) ||
            (!value.usenetUsername && value.usenetPassword)
        ) {
            context.addIssue({
                code: "custom",
                ...
                message:
                    "Enter both the username and password, or leave both blank to keep saved credentials.",
            });
        }
    });

const traktFormSchema = z
    .object({
        ...
        traktAccessToken: z
            .string()
            .trim()
            .max(512, "OAuth token must be 512 characters or fewer."),
    })
    .superRefine((value, context) => {
        if (
            (value.traktClientId && !value.traktAccessToken) ||
            (!value.traktClientId && value.traktAccessToken)
        ) {
            context.addIssue({
                code: "custom",
                ...
                message:
                    "Enter both the client ID and OAuth token, or leave both blank to keep saved credentials.",
            });
        }
    });
...
export function prepareConnectionFormValues(
    ...
    if (serviceType === "trakt") {
        ...
        return {
            ...
            apiKey:
                parsed.data.traktClientId && parsed.data.traktAccessToken
                    ? JSON.stringify({
...
export function getUsenetFormDefaults(baseUrl: string): UsenetFormDefaults {
    try {
        ...
        // connection to nntps://.
        const port = url.protocol === "nntps:" && url.port ? Number.parseInt(url.port, 10) : 563;
