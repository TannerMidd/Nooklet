        ...
        <div className="space-y-2">
            ...
            {intent === "test-save" ? (
                <p className="text-sm text-muted" role="status">
                    Testing the draft with {displayName}. The saved connection stays active if this
                    test fails.
                </p>
            ) : null}
            {intent === "save" ? (
...
function FieldError({ id, message }: { id: string; message?: string }) {
    return message ? (
        <p id={id} className="text-sm text-accent-wine">
            {message}
        </p>
    ) : null;
}
    ...
    return (
        <div className="space-y-3.5">
            ...
            <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_140px]">
                ...
                <label className="space-y-2">
                    ...
                    <Input
                        ...
                        aria-describedby={
                            errors?.usenetConnections ? "usenet-connections-error" : undefined
                        }
                    />
            ...
            <div className="grid gap-3 sm:grid-cols-2">
                <label className="space-y-2">
                    ...
                    <Input
                        ...
                        aria-describedby={
                            errors?.usenetUsername ? "usenet-username-error" : undefined
                        }
                    />
                ...
                <label className="space-y-2">
                    ...
                    <Input
                        ...
                        aria-describedby={
                            errors?.usenetPassword ? "usenet-password-error" : undefined
                        }
                    />
            ...
            <p className="text-[13px] leading-5 text-muted">
                Nooklet stores these fields securely and only connects over TLS, so downloads and
                credentials are never readable on the network. Use your provider&apos;s TLS port
                (usually 563).
            </p>
        </div>
    );
    ...
    return (
        <div className="grid gap-3 sm:grid-cols-2">
            ...
            <p className="text-[13px] leading-5 text-muted sm:col-span-2">
                Create a Trakt API application, then paste its client ID and an OAuth access token
                here.
            </p>
        </div>
    );
...
function PrivateNetworkHelp({
    serviceType,
}: {
    serviceType: ServiceConnectionSummary["serviceType"];
}) {
    ...
    return (
        <div className="rounded-lg border border-cream/[0.08] bg-cream/[0.03] px-3.5 py-3 text-[13px] leading-5 text-muted">
            ...
            <p className="mt-1">
                <code>localhost</code> points to the Nooklet container. Use the Compose service
                name, or <code>host.docker.internal</code> for a service running on the host.
            </p>
            <p className="mt-2">
                For a LAN address, add only that exact hostname or IP to{" "}
                <code>PRIVATE_SERVICE_HOST_ALLOWLIST</code> and restart Nooklet. Avoid enabling the
                broad private-network override.
            </p>
        </div>
    );
}
    ...
    return (
        <>
            ...
            >
                ...
                {summary.status !== "disconnected" ? (
                    <dl className="space-y-2 border-t border-cream/[0.07] pt-3.5">
                        <ConnectionFact
                            label="Secret"
                            value={summary.maskedSecret ?? "Not configured"}
                        />
                        {showsModel ? (
                            <ConnectionFact
                                label={definition.modelLabel ?? "Model"}
                                value={summary.model ?? "Not set"}
                            />
                        ) : null}
                        {showsModel ? (
                            <ConnectionFact
                        ...
                        {showsAvailableUsers ? (
                            <ConnectionFact
                                label="Users"
                                value={
                                    summary.availableUsers.length > 0
                                        ? summary.availableUsers.length
                                        : "Run verify"
                                }
                            />
                        ) : null}
                        <ConnectionFact
                            label="Verified"
                            value={formatDate(summary.lastVerifiedAt)}
                        />
                    </dl>
                ) : null}
            ...
            <AlertDialog
                ...
                description={
                    <p>
                        Nooklet will remove the saved credential and verification state. Features
                        that depend on this service will stop working until it is connected again.
                    </p>
                }
