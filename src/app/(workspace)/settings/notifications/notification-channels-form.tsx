            ...
            <form action={action} className="space-y-4 border-t border-cream/[0.07] p-4">
                ...
                <label className="block space-y-1.5 text-sm">
                    ...
                    <Input
                        name="targetUrl"
                        type="url"
                        placeholder="Leave blank to keep the current secret URL"
                    />
                    <span className="block text-xs text-muted">
                        Current: {channel.maskedTargetUrl}
                    </span>
                </label>
                <fieldset className="space-y-2">
                    ...
                    <div className="grid gap-2 md:grid-cols-2">
                        {notificationEventTypes.map((event) => (
                            <label
                                key={event}
                                className="flex min-h-11 items-center gap-3 rounded-lg border border-cream/[0.10] px-3 py-2 text-sm text-foreground"
                            >
                                <input
                                    type="checkbox"
                                    name="events"
    ...
    return (
        <li className="space-y-3 rounded-xl border border-cream/[0.08] bg-cream/[0.04] p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted">
                        ...
                        <span className="break-all">{channel.maskedTargetUrl}</span>
                    </p>
                </div>
            ...
            {channel.lastDispatchAt ? (
                <p className="text-xs text-muted">
                    Last delivery{" "}
                    {channel.lastDispatchStatus === "success" ? "succeeded" : "failed"} at{" "}
                ...
                <form action={addAction} className="space-y-4">
                    <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <label className="space-y-1.5 text-sm">
                            ...
                            <select
                                ...
                                value={selectedType}
                                onChange={(event) =>
                                    setSelectedType(event.target.value as NotificationChannelType)
                                }
                    ...
                    <label className="space-y-1.5 text-sm">
                        ...
                        <Input
                            name="targetUrl"
                            type="url"
                            placeholder={channelTypePlaceholders[selectedType]}
                            required
                        />
                    </label>
                    <fieldset className="space-y-2">
                        ...
                        <div className="grid gap-2 md:grid-cols-2">
                            {notificationEventTypes.map((event) => (
                                <label
                                    key={event}
                                    className="flex min-h-11 items-center gap-3 rounded-lg border border-cream/[0.10] bg-cream/[0.04] px-3 py-2 text-sm text-foreground"
                                >
                                    <input
                                        type="checkbox"
                                        name="events"
