...
async function mapSearchResults(
    userId: string,
    results: Array<{
        id: string;
...
export async function requestSearchTitleAction(
    ...
    try {
        ...
        const primarySelection =
            requested.selections.length === 1 ? (requested.selections[0] ?? null) : null;
        ...
        const searchedSelection = requested.selections.find(
            (selection) => selection.releaseSearch.searched,
        );
        const queuedSelection = requested.selections.find(
        ...
        return {
            ...
            results:
                showManualCandidates && releaseSearch.searched
