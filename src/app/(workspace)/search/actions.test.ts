...
vi.mock("@/modules/indexers/workflows/search-indexers", async (importOriginal) => {
    const actual =
        await importOriginal<typeof import("@/modules/indexers/workflows/search-indexers")>();
...
vi.mock("@/modules/discover/queries/search-discover-titles", async (importOriginal) => {
    const actual =
        await importOriginal<typeof import("@/modules/discover/queries/search-discover-titles")>();
...
vi.mock("@/modules/downloads/workflows/queue-indexer-result", async (importOriginal) => {
    const actual =
        await importOriginal<typeof import("@/modules/downloads/workflows/queue-indexer-result")>();
...
import { queueIndexerResultAction, requestSearchTitleAction, searchTitlesAction } from "./actions";
import {
...
describe("searchTitlesAction", () => {
    ...
    it("returns safe title search result metadata", async () => {
        ...
        expect(discoverSearchMock).toHaveBeenCalledWith("u1", {
            mediaType: "movie",
            query: "Arrival",
        });
...
describe("requestSearchTitleAction", () => {
    ...
    it("adds a title and revalidates library pages", async () => {
        ...
        const result = await requestSearchTitleAction(
            initialRequestSearchTitleActionState,
            validForm(),
        );

        expect(requestTitleWorkflowMock).toHaveBeenCalledWith(
            "u1",
    ...
    it("returns release candidates when no result matches the quality profile", async () => {
        ...
        expect(result).toMatchObject({
            ...
            outcome: "no_match",
            message:
                "Arrival was added to your catalog, but no release matched HD 1080p. You can search again from its library page.",
            titleId: "title1",
            searchRunId: "run1",
            downloadRequestId: null,
            results: [{ id: "result1" }],
        ...
        requestTitleWorkflowMock.mockResolvedValue({
            ...
            selections: [
                ...
                },
                {
                    ...
                    releaseSearch: {
                        searched: true,
                        searchRun: { id: "run-s2", status: "succeeded" },
                        results: [],
                    },
                    queuedDownload: { queued: false, reason: "no_matching_release" },
                },
...
describe("queueIndexerResultAction", () => {
    ...
    it("returns sign-in error when there is no session", async () => {
        ...
        const result = await queueIndexerResultAction(
            initialQueueIndexerResultActionState,
            validForm(),
        );
