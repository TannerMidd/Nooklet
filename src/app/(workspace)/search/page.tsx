...
import { parseSearchPageParams, type SearchPageParams } from "@/app/(workspace)/search/search-page-state";
  ...
  const initialState = !params.query
    ...
    : !parsedSearch.success
      ? {
          ...
          message: parsedSearch.error.issues[0]?.message ?? "Review the search and try again.",
          results: [],
        }
      : titleSearch?.ok
        ? {
