...
async function refreshDownloadStorageSnapshot(
  id: string,
  ...
  try {
    const [filesystem, readable, writable] = await Promise.all([
      ...
      access(inspectionPath, constants.R_OK).then(() => true, () => false),
      access(inspectionPath, constants.W_OK).then(() => true, () => false),
    ]);
...
export async function refreshLibraryDestinationStorageSnapshot(
  ...
  try {
    const [filesystem, readable, writable] = await Promise.all([
      ...
      access(effectivePath, constants.R_OK).then(() => true, () => false),
      access(effectivePath, constants.W_OK).then(() => true, () => false),
    ]);
