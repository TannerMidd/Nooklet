...
export async function pruneOperationalHistory(
    ...
    return database.transaction((tx) => ({
        expiredAuthSessions: tx.delete(authSessions).where(lte(authSessions.expiresAt, now)).run()
            .changes,
        auditEvents: tx.delete(auditEvents).where(lt(auditEvents.createdAt, cutoff)).run().changes,
        notificationDispatches: tx
            .delete(notificationDispatchAudit)
            .where(lt(notificationDispatchAudit.dispatchedAt, cutoff))
            .run().changes,
        recommendationTimelineEvents: tx
            .delete(recommendationItemTimelineEvents)
            .where(lt(recommendationItemTimelineEvents.createdAt, cutoff))
            .run().changes,
        watchHistorySyncRuns: tx
            .delete(watchHistorySyncRuns)
            .where(
                and(
