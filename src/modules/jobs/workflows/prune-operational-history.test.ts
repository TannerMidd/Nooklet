...
describe("pruneOperationalHistory", () => {
    it("removes expired operational audit rows and keeps recent rows", async () => {
        ...
        expect(
            database.select().from(authSessions).where(eq(authSessions.id, expiredSessionId)).get(),
        ).toBeUndefined();
        expect(result.auditEvents).toBeGreaterThanOrEqual(1);
        expect(
            database.select().from(auditEvents).where(eq(auditEvents.id, oldId)).get(),
        ).toBeUndefined();
        expect(
            database.select().from(auditEvents).where(eq(auditEvents.id, recentId)).get()?.id,
        ).toBe(recentId);
    });
});
