...
export async function listJobsForUser(userId: string, jobType?: JobType) {
    ...
    return database
        ...
        .where(
            jobType
                ? and(eq(jobs.userId, userId), eq(jobs.jobType, jobType))
                : eq(jobs.userId, userId),
        )
...
export async function saveRecurringJob(input: SaveRecurringJobInput) {
    ...
    return database.transaction((transaction) => {
        ...
        if (existingJob) {
            ...
            const shouldClearExecutionState =
                !activeLease && (!input.isEnabled || existingJob.lastStatus === "running");
...
function finishJobRun(input: {
    ...
    database.transaction((transaction) => {
        ...
        const hasDeferredImmediateRun =
            current.scheduleMinutes === 0 &&
            current.isEnabled &&
