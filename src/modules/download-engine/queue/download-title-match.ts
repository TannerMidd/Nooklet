import { type DownloadQueueItem, type DownloadQueueSnapshot } from "./download-queue";
import { type RecommendationProviderMetadata } from "@/modules/recommendations/provider-metadata";

type DownloadTitleMatchInput = {
  title: string;
  year?: number | null;
...
function buildTitleCandidates(input: DownloadTitleMatchInput) {
  const candidates = [
    input.title,
  ...
  return candidates.flatMap((candidate) => {
    ...
    if (normalizedCandidate.length < 4 || seen.has(normalizedCandidate)) return [];
    seen.add(normalizedCandidate);
    return [normalizedCandidate];
  });
}

function scoreQueueItem(item: DownloadQueueItem, input: DownloadTitleMatchInput) {
  ...
  return buildTitleCandidates(input).reduce((bestScore, candidateKey) => {
    let score = 0;
    if (queueKey === candidateKey) score = 100;
    else if (queueKey.startsWith(`${candidateKey} `)) score = 85;
    else if (queueKey.includes(` ${candidateKey} `)) score = 75;
    else if (queueKey.includes(candidateKey)) score = 65;
    else {
    ...
    if (score > 0 && yearToken && queueTokens.has(yearToken)) score += 12;
    return Math.max(bestScore, score);
  }, 0);
}

export function findDownloadQueueItemForTitle(
  snapshot: DownloadQueueSnapshot | null | undefined,
  input: DownloadTitleMatchInput,
) {
  if (!snapshot?.items.length) return null;

  return snapshot.items
    .map((item) => ({ item, score: scoreQueueItem(item, input) }))
