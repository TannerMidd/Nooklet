...
describe("NntpClient", () => {
    ...
    it("maps RFC transient service responses to server-unavailable", async () => {
        const scripted = await startScriptedTlsServer("200 scripted ready", "403 temporary fault");
        ...
        try {
            ...
            await expect(nntp.body("temporary@test")).rejects.toMatchObject({
    ...
    it("maps a valid 502 response to permanent protocol-error", async () => {
        const scripted = await startScriptedTlsServer(
            "200 scripted ready",
        ...
        try {
            ...
            await expect(nntp.body("forbidden@test")).rejects.toMatchObject({
                kind: "protocol-error",
                permanent: true,
            });
            nntp.destroy();
