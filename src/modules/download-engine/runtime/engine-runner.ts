...
import {
    ...
    listEngineDownloadArtifactStates,
...
const minimumFetchingStageBudgetMs = 30 * 60_000;
/** Conservative throughput assumption for sizing a transfer's time budget. */
const fetchingStageAssumedBytesPerSecond = 512 * 1024;
...
export function fetchingStageBudgetMs(totalBytes: number): number {
    const configuredOverride = env.DOWNLOAD_STAGE_BUDGET_MS;
    ...
    const transferAllowanceMs = Math.ceil(safeBytes / fetchingStageAssumedBytesPerSecond) * 1_000;

    return minimumFetchingStageBudgetMs + transferAllowanceMs;
}
