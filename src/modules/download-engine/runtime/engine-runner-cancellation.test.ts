...
beforeEach(() => {
  ...
  mocks.claimQueuedEngineDownload.mockImplementation(
    async (id: string) => (id === download.id ? download : null),
  );
...
describe("engine runner durable cancellation fencing", () => {
  ...
  it("skips a download that does not fit and runs the next one that does", async () => {
    const oversized = { ...download, id: "22222222-2222-4222-8222-222222222222", totalBytes: 900 };
    ...
    mocks.claimQueuedEngineDownload.mockImplementation(async (id: string) => {
      if (id !== fits.id) return null;
      claimed.add(id);
      return fits;
    });
    mocks.inspectLiveEngineCapacity.mockImplementation(
      async (candidate: { totalBytes: number }) => ({ sufficient: candidate.totalBytes < 100 }),
    );
