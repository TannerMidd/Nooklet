 ...
 */
export async function inspectLiveEngineCapacity(
  candidate: { totalBytes: number },
  ...
  const candidateBytes = Number.isSafeInteger(candidate.totalBytes) && candidate.totalBytes > 0
    ? candidate.totalBytes * workspaceMultiplier
    : 0;
  const requiredBytes = minimumFreeSpaceReserveBytes
    + usage.activeWorkspaceBytes
    + candidateBytes;
