...
async function removeItem(userId: string, itemId: string): Promise<EngineQueueActionOutcome> {
    ...
    try {
        for (const entry of entries) {
            ...
            if (!fulfillmentId || workLeases.has(fulfillmentId)) {
                continue;
            }
        ...
        for (const entry of entries) {
            if (entry.request.fulfillmentId || requestWorkLeases.has(entry.request.id)) {
                continue;
            }
        ...
        for (const entry of entries) {
            ...
            if (!fulfillmentId || cancellationCheckpoints.has(fulfillmentId)) {
                continue;
            }
            ...
            if (checkpoint) {
                cancellationCheckpoints.set(fulfillmentId, checkpoint);
            }
        }
        ...
        await Promise.all([
            ...
            ...[...requestWorkLeases.values()].map((lease) =>
                releaseDownloadRequestWorkLease(lease),
            ),
        ]);
    }
}
