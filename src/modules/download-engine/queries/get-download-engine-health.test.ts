...
describe("download engine health", () => {
    it("distinguishes idle from active healthy work", () => {
        expect(
            evaluateDownloadEngineHealth([], emptyDownloadEngineLoopHealth(), now.getTime()).status,
        ).toBe("idle");

        expect(
            evaluateDownloadEngineHealth([row()], emptyDownloadEngineLoopHealth(), now.getTime()),
        ...
        const parked = evaluateDownloadEngineHealth(
            [
                row({
                    ...
                    failureKind: "infrastructure",
                    errorMessage:
                        "The news server kept failing on articles this release does have.",
                }),
            ],
            emptyDownloadEngineLoopHealth(),
            now.getTime(),
