...
describe("detectFileKind", () => {
  ...
  it("detects RAR4 and RAR5 volumes", () => {
    ...
    expect(detectFileKind(header([0x52, 0x61, 0x72, 0x21, 0x1a, 0x07, 0x01, 0x00])).kind).toBe("rar");
  });
  ...
  it("detects MP4 via ftyp at offset 4", () => {
    expect(detectFileKind(header([0x00, 0x00, 0x00, 0x20, 0x66, 0x74, 0x79, 0x70, 0x69, 0x73, 0x6f, 0x6d]))).toEqual({
      kind: "video",
      extension: ".mp4",
    });
  });
  ...
  it("detects AVI (RIFF + AVI )", () => {
    expect(
      detectFileKind(header([0x52, 0x49, 0x46, 0x46, 0x11, 0x22, 0x33, 0x44, 0x41, 0x56, 0x49, 0x20])),
