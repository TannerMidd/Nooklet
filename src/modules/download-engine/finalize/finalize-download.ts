...
function assertSafeArchiveEntry(entryName: string) {
    ...
    if (
        ...
        throw new FinalizeDownloadError(
            `Archive contains an unsafe path: ${entryName.slice(0, 160)}`,
        );
    }
}
...
function assertNoArchiveLinkMetadata(fields: ReadonlyMap<string, string>, entryName: string) {
    for (const [rawKey, rawValue] of fields) {
        ...
        const linkKey =
            /(?:symbolic|sym|hard)\s*link|^link$|link\s*target|redir(?:ection)?|junction|reparse|^target$/.test(
                key,
            );
        const linkValue = /symbolic\s+link|symlink|hard\s+link|junction|reparse(?:\s+point)?/.test(
            value,
        );
        const unixLinkMode =
            /(?:attribute|mode|type)/.test(key) && /(?:^|\s)l[rwxst-]{9}(?:\s|$)/.test(value);
        const genericLinkType = key === "type" && value === "link";

        if (
            (linkKey && isMeaningfulMetadata(value)) ||
            linkValue ||
            unixLinkMode ||
            genericLinkType
        ) {
...
function parseNonNegativeSize(value: string | undefined, entryName: string) {
    if (!value || !/^\d+$/.test(value.trim())) {
        throw new FinalizeDownloadError(
            `Archive has an invalid declared size for: ${entryName.slice(0, 160)}`,
        );
    }
...
function parseTechnicalRecords(
    ...
    for (const line of listing.split(/\r?\n/)) {
        ...
        if (key.toLowerCase() === entryKey.toLowerCase()) {
            ...
            if (duplicateKey) {
                throw new FinalizeDownloadError(
                    `Archive technical listing repeats the ${key.slice(0, 80)} field.`,
                );
            }
...
export function parse7zTechnicalListing(listing: string): ArchiveListingEntry[] {
    ...
    return records.map((fields) => {
        ...
        const kind: ArchiveListingEntry["kind"] =
            folder || /^d/i.test(attributes) ? "directory" : "file";
...
export async function verifyStagedArchiveTree(
    ...
    function assertWithinQuota() {
        if (
            ...
            throw new FinalizeDownloadError(
                "Extracted output exceeds the configured safety quota.",
            );
        }
    }
    ...
    async function visit(currentDir: string): Promise<void> {
        for (const entry of await readdir(currentDir, { withFileTypes: true })) {
            ...
            if (entryStats.isSymbolicLink()) {
                throw new FinalizeDownloadError(
                    `Archive extracted a link or reparse entry: ${entry.name.slice(0, 160)}`,
                );
            }
            ...
            if (!isContainedPath(resolvedRoot, resolvedEntry)) {
                throw new FinalizeDownloadError(
                    `Archive output escaped its staging directory: ${entry.name.slice(0, 160)}`,
                );
            }
            ...
            if (entryStats.isDirectory()) {
                ...
                if (entryStats.nlink > 1) {
                    throw new FinalizeDownloadError(
                        `Archive extracted a hard link: ${entry.name.slice(0, 160)}`,
                    );
                }
                ...
                throw new FinalizeDownloadError(
                    `Archive extracted a non-regular file: ${entry.name.slice(0, 160)}`,
                );
            }
        }
    }
...
async function assertMergeHasNoCollisions(sourceDir: string, targetDir: string): Promise<void> {
    for (const entry of await readdir(sourceDir, { withFileTypes: true })) {
        ...
        if (
            ...
            throw new FinalizeDownloadError(
                `Archive merge source is not a regular tree: ${entry.name.slice(0, 160)}`,
            );
        }
        ...
        if (!sourceStats.isDirectory()) {
            if (targetStats) {
                throw new FinalizeDownloadError(
                    `Archive output collides with an existing path: ${entry.name.slice(0, 160)}`,
                );
            }
...
async function extractInspectedArchives(
    ...
    await withArchiveStaging(stagingParent, "archive-merge", async (combinedStagingDir) => {
        for (const [index, plan] of plans.entries()) {
            await withArchiveStaging(
                stagingParent,
                `archive-${index + 1}`,
                async (archiveStagingDir) => {
                    try {
                        if (plan.tool === "unrar") {
                            await execFileAsync(
                                "unrar",
...
async function runPar2(workDir: string, warnings: string[]): Promise<Par2Outcome> {
    ...
    if (!(await binaryAvailable("par2"))) {
        warnings.push(
            "par2 is not installed — skipped verification/repair and file-name restoration.",
        );
