...
describe("evaluateReadiness", () => {
  ...
  it("keeps optional integrations from blocking first-run completion", () => {
    ...
    const result = evaluateReadiness(input);
    expect(result.setupComplete).toBe(true);
    expect(result.capabilities.find((entry) => entry.id === "recommendations")?.status).toBe("optional");
    expect(result.capabilities.find((entry) => entry.id === "watch-history")?.status).toBe("optional");
  });
  ...
  it("does not call a writable workspace ready when reservations consume its capacity", () => {
    ...
    const result = evaluateReadiness(input);
    expect(result.capabilities.find((entry) => entry.id === "storage")?.status).toBe("needs-attention");
    expect(result.capabilities.find((entry) => entry.id === "movie-downloads")?.details)
