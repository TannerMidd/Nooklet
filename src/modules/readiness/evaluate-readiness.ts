...
export function evaluateReadiness(input: ReadinessEvaluationInput): ReadinessEvaluation {
    const serviceStatus = new Map(
        input.services.map((service) => [service.serviceType, service.status]),
    );
    const verified = (serviceType: ServiceConnectionType) =>
        serviceStatus.get(serviceType) === "verified";
    ...
    const workspaceReady =
        input.downloadWorkspace.reachable &&
    ...
    const destinationReady = (mediaType: RecommendationMediaType) =>
    ...
    const downloadCapability = (mediaType: RecommendationMediaType): ReadinessCapability => {
        ...
        if (!downloaderReady) {
            missing.push("Connect and verify a Usenet server for the built-in downloader.");
        }

        if (!indexerReady(mediaType)) {
            missing.push(
                `Verify an indexer with ${mediaType === "movie" ? "movie" : "TV"} categories.`,
            );
        }

        if (!destinationReady(mediaType)) {
            missing.push(
                `Add a reachable, writable ${mediaType === "movie" ? "movie" : "TV"} library destination.`,
            );
        }

        if (!builtInStorageReady) {
            missing.push("Make the built-in download workspace writable and free enough space.");
        }

        if (!workerReady) {
            missing.push(
                "Restore the background worker so downloads can be processed and imported.",
            );
        }
    ...
    const finalDestinationsHealthy =
    ...
    const capabilities: ReadinessCapability[] = [
        ...
        capability({
            ...
            details: [
                ...(workspaceRequired && !workspaceReady
                    ...
                    : []),
                ...(input.destinations.length === 0
                    ? ["No final library destination is configured."]
                    : []),
        ...
        capability({
            ...
            title: "Watch history",
            status:
                input.watchHistory.sourceCount > 0 || input.watchHistory.itemCount > 0
                    ? "ready"
                    : "optional",
            required: false,
            summary:
                input.watchHistory.itemCount > 0
        ...
        capability({
            ...
            required: false,
            summary:
                input.notifications.enabledCount > 0
            ...
            details:
                input.notifications.configuredCount > 0 && input.notifications.enabledCount === 0
    ...
    const readyForFirstRequest =
        movieDownloads.status === "ready" || tvDownloads.status === "ready";
