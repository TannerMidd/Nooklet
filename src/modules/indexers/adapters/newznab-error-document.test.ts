...
import { detectNewznabErrorDocument, formatNewznabErrorDocument } from "./newznab-error-document";
...
describe("detectNewznabErrorDocument", () => {
    ...
    it.each([
        ...
        expect(
            detectNewznabErrorDocument(`<error code="${code}" description="${description}"/>`),
    ...
    it("treats an HTML body as an indexer fault", () => {
        expect(
            detectNewznabErrorDocument("<!DOCTYPE html><html><body>Log in</body></html>"),
    ...
    it("still reports an error element that declares only a description", () => {
        expect(detectNewznabErrorDocument(`<error description="Temporarily offline"/>`)).toEqual({
            code: null,
    ...
    it.each([
        [
            ...
            `<rss><channel><item><title>Arrival 2016</title></item></channel></rss>`,
        ],
...
describe("formatNewznabErrorDocument", () => {
    it("includes the code when the indexer declared one", () => {
        expect(
            formatNewznabErrorDocument({ code: 910, description: "Request limit reached" }),
        ).toBe("Indexer error 910: Request limit reached");
    });

    it("uses the description alone when there is no code", () => {
        expect(formatNewznabErrorDocument({ code: null, description: "Temporarily offline" })).toBe(
            "Temporarily offline",
        );
    });
});
