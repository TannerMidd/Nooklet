...
describe("removeIndexerCommand", () => {
  ...
  it("does not remove a shared indexer that the caller does not own", async () => {
    findMock.mockResolvedValue({ id: "indexer-1", userId: "admin-1", name: "NZBGeek" } as never);

    await expect(removeIndexerCommand("user-1", "indexer-1"))
      .rejects.toBeInstanceOf(RemoveIndexerCommandError);
