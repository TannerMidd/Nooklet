...
describe("disconnectServiceConnection", () => {
    ...
    it.each([["ai-provider"], ["tautulli"], ["plex"], ["usenet-server"]] as const)(
        "forwards the %s service type to the repository and audit subject",
        async (serviceType) => {
            deleteMock.mockResolvedValue(true);
            ...
            expect(auditMock.mock.calls[0]?.[0]?.payloadJson).toBe(JSON.stringify({ serviceType }));
        },
    );
