...
export type SearchTmdbTitlesResult =
    { ok: true; titles: TmdbTitleSearchResult[] } | { ok: false; message: string };
...
export type LookupTmdbTitleDetailsResult =
    { ok: true; details: TmdbTitleDetails } | { ok: false; message: string };
...
function shouldUseBearerAuth(secret: string) {
    ...
    return (
        secret.trim().toLowerCase().startsWith("bearer ") ||
        normalizedSecret.split(".").length === 3
    );
}
...
function getCandidateOriginalTitle(
...
export async function lookupTmdbTitleDetails(
    input: TmdbConnectionInput & {
        mediaType: RecommendationMediaType;
        title: string;
        year: number | null;
    },
...
export async function lookupTmdbTitleDetailsByTmdbId(
    input: TmdbConnectionInput & {
        mediaType: RecommendationMediaType;
        tmdbId: number;
    },
    ...
    const detailsPath =
        input.mediaType === "movie" ? `movie/${input.tmdbId}` : `tv/${input.tmdbId}`;
...
export type LookupTmdbTvSeasonsResult =
    { ok: true; seasons: TmdbTvSeasonSummary[] } | { ok: false; message: string };

type TmdbTvSeasonsPayload = {
    seasons?: unknown;
};

function normalizeTmdbTvSeason(
    value: unknown,
    imageBaseUrl: string | null,
...
export async function lookupTmdbTvSeasons(
    input: TmdbConnectionInput & {
        tmdbId: number;
    },
...
export async function lookupTmdbTvSeasonEpisodes(
    input: TmdbConnectionInput & {
        tmdbId: number;
        seasonNumber: number;
    },
...
export type ListTmdbDiscoverTitlesResult =
    { ok: true; titles: TmdbDiscoverTitle[] } | { ok: false; message: string };
    ...
    for (const entry of payload.results) {
        ...
        titles.push({
            ...
            posterUrl: buildImageUrl(
                imageBaseUrl,
                (entry as { poster_path?: unknown }).poster_path,
                "w500",
            ),
...
export async function listTmdbDiscoverTitles(
    input: TmdbConnectionInput & {
    ...
    for (const entry of payload.results) {
        ...
        titles.push({
            ...
            backdropUrl: buildImageUrl(
                imageBaseUrl,
                (entry as { backdrop_path?: unknown }).backdrop_path,
                "w780",
            ),
            releaseDate,
