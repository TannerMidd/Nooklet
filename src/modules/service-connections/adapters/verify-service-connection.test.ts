...
describe("verifyServiceConnection dispatcher", () => {
    ...
    it.each([
        ...
        "routes %s to the correct verifier and returns its result verbatim",
        async (serviceType, getMock) => {
    ...
    it("returns a typed failure for an unsupported service type without invoking any verifier", async () => {
        const result = await verifyServiceConnection(
            buildInput({
                serviceType:
                    "totally-unknown" as unknown as VerifyServiceConnectionInput["serviceType"],
            }),
        );
