...
vi.mock("@/lib/security/safe-fetch", () => ({
    assertOutboundHostAllowed: vi.fn().mockResolvedValue([{ address: "203.0.113.10", family: 4 }]),
}));
...
describe("verifyUsenetServer", () => {
    ...
    it("surfaces dial failures as a safe verification error", async () => {
        clientMocks.connect.mockRejectedValue(
            new Error("Could not connect to news.example.test:563."),
        );
