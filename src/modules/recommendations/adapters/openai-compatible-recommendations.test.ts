...
describe("openai-compatible-recommendations", () => {
    ...
    it("emphasizes selected genres and filtered-library mixing in the user prompt", () => {
        ...
        expect(prompt).toContain(
            "Original-language requirement: every recommendation must have English (en) as its original language.",
        );
        expect(prompt).toContain(
            "These selected genres are the strongest instruction for this request.",
        );
        expect(prompt).toContain("include a mix across all selected genres when possible");
        expect(prompt).toContain(
            "Owned library sample: 1 of 8 known movies matching the selected genres.",
        );
    });
    ...
    it("sends the emphasized genre prompt to the AI provider", async () => {
        ...
        expect(requestBody.messages[1]?.content).toContain(
            "Every recommendation must align with at least one selected genre",
        );
    });
