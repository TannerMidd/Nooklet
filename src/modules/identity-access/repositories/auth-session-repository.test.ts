...
async function issueForUser(
  ...
  if (!session) throw new Error("Expected the test session to be issued.");
  return session;
}
...
describe("auth-session-repository", () => {
  ...
  it("revokes every active session atomically when an administrator disables a user", async () => {
    ...
    await expect(updateUserDisabledStateGuarded(actor.id, target.id, true)).resolves.toMatchObject({
      status: "updated",
  ...
  it("rejects a pending login after disablement and re-enablement", async () => {
    ...
    await expect(issueAuthSession(
      target.id,
      authenticatedGeneration,
      authenticatedPasswordChangedAt,
    )).resolves.toBeNull();
  });
