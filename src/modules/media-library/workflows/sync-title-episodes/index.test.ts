...
function tmdbEpisode(seasonNumber: number, episodeNumber: number, name: string, airDate: string | null) {
  return {
    seasonNumber,
    episodeNumber,
    name,
...
describe("syncTitleEpisodesWorkflow", () => {
  ...
  it("only fetches episodes for scoped seasons but persists every season row", async () => {
    ...
    const seasons = await listTvSeasonsForTitle(title.id);
    expect(seasons.map((season) => ({ seasonNumber: season.seasonNumber, monitored: season.monitored }))).toEqual([
  ...
  it("preserves existing monitoring choices on refresh while inserting new rows", async () => {
    const { userId, title } = await seedTitle();
    const season = await createTvSeason({ titleId: title.id, seasonNumber: 1, monitored: false });
