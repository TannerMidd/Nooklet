...
import { listCancellableSeasonFulfillmentsForTitle } from "@/modules/downloads/public";
...
export async function retireMediaTitlePreservingFilesWorkflow(
    ...
    for (const request of requests) {
        ...
        if (checkpoint?.cancellationRequestedAt && !request.cancellationRequestedAt) {
            cancellationCheckpointCount += 1;
        }
    }
    ...
    try {
        ...
        if (error instanceof RemoveMediaTitleCommandError && error.code === "active_download") {
        ...
        if (error instanceof RemoveMediaTitleCommandError && error.code === "title_not_found") {
            return {
                status: "removed",
                removedTitle: null,
                cancellationCheckpointCount,
