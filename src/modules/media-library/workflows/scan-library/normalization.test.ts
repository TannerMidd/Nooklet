...
function fetched(
    sourceRecord: ActiveMediaLibraryPathRecord,
    relativePath: string,
): FetchedLibrarySources {
    return {
...
describe("normalizeLibraryFiles", () => {
    it("groups TV episodes by show folder and extracts season and episode numbers", () => {
        ...
        const result = normalizeLibraryFiles(
            fetched(
                tvSource,
    ...
    it("uses the first folder as the TV show when no season folder exists", () => {
        const tvSource = source("tv", path.join("E:\\", "Plex Media", "TV Shows"));
        const result = normalizeLibraryFiles(
            fetched(tvSource, "Severance/Severance.S01E02.2160p.WEB-DL.mkv"),
        );
    ...
    it("keeps movie files as movie titles without TV episode metadata", () => {
        ...
        const result = normalizeLibraryFiles(
            fetched(movieSource, "Arrival (2016)/Arrival.2016.1080p.BluRay.mkv"),
        );
    ...
    it("preserves Unicode letters in stable title identities", () => {
        const movieSource = source("movie", path.join("F:\\", "Movies"));
        const result = normalizeLibraryFiles(
            fetched(movieSource, "千と千尋の神隠し (2001)/千と千尋の神隠し.2001.1080p.mkv"),
        );
