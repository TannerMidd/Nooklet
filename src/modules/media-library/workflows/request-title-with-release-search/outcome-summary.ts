...
export type RequestSubmissionOutcome =
    "catalog_added" | "queued" | "partial_queue" | "no_match" | "search_failed" | "queue_failed";
...
function selectionIsFulfilled(
    ...
    if (selection.queuedDownload.queued) {
        return true;
    }
...
export function summarizeRequestSubmission(
    ...
    const queueResults =
        input.result.selections.length > 0
    ...
    if (queuedCount === selectionCount) {
        ...
        if (fallbacks.length > 0) {
            const packCount = input.result.selections.filter(
                (selection) => selection.queuedDownload.queued,
            ).length;
            const queuedEpisodes = fallbacks.reduce(
                (total, fallback) => total + fallback.queuedCount,
                0,
            );
            const activeEpisodes = fallbacks.reduce(
                (total, fallback) => total + fallback.activeCount,
                0,
            );
            const ownedEpisodes = fallbacks.reduce(
                (total, fallback) => total + fallback.ownedCount,
                0,
            );
    ...
    const outcome: RequestSubmissionOutcome = failedResults.every(
        (result) => result.reason === "no_matching_release",
    )
        ? "no_match"
        : failedResults.some(
                (result) => result.reason === "search_failed" || result.reason === "search_not_run",
            )
          ? "search_failed"
          : "queue_failed";
    ...
    return {
        ...
        status: "warning",
        message:
            selectionCount > 1 && issues.length > 0 ? `${message} ${issues.join(" ")}` : message,
        queuedCount: 0,
        selectionCount,
    };
}
