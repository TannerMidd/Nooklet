...
export async function findMediaLibraryByIdForUser(userId: string, libraryId: string) {
    ...
    return (
        database
            ...
            .from(mediaLibraries)
            .where(
                and(eq(mediaLibraries.userId, instanceOwnerId), eq(mediaLibraries.id, libraryId)),
            )
            .get() ?? null
    );
}
        ...
        database
            ...
            .where(
                and(
                    ...
                    eq(mediaLibraryPaths.id, pathId),
                ),
            )
            .get() ?? null
    );
...
export async function listActiveMediaLibraryPaths(
    userId: string,
): Promise<ActiveMediaLibraryPathRecord[]> {
    const database = ensureDatabaseReady();
...
export async function setDefaultDownloadPath(input: {
    ...
    const row = database
        ...
        .innerJoin(mediaLibraries, eq(mediaLibraries.id, mediaLibraryPaths.libraryId))
        .where(
            and(eq(mediaLibraryPaths.userId, input.userId), eq(mediaLibraryPaths.id, input.pathId)),
        )
        .get();
    ...
    for (let offset = 0; offset < fileIds.length; offset += 500) {
        database
            ...
            .where(
                and(
                    eq(mediaFiles.userId, userId),
                    inArray(mediaFiles.id, fileIds.slice(offset, offset + 500)),
                ),
            )
            .run();
    }
...
export async function reconcileMediaTitleFileAvailability(userId: string, titleId: string) {
    ...
    const episodeIds = Array.from(
        new Set(files.flatMap((file) => (file.episodeId ? [file.episodeId] : []))),
    );
    const updatedAt = new Date();
...
export async function upsertTvSeason(input: {
    ...
    return database
        ...
        .where(
            and(
                eq(tvSeasons.titleId, input.titleId),
                eq(tvSeasons.seasonNumber, input.seasonNumber),
            ),
        )
        .get()!;
}
...
export async function setTvEpisodeHasFile(input: { episodeId: string; hasFile: boolean }) {
    ...
    return (
        database.select().from(tvEpisodes).where(eq(tvEpisodes.id, input.episodeId)).get() ?? null
    );
}
