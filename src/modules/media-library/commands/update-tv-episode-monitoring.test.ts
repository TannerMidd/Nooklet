...
describe("updateTvEpisodeMonitoringCommand", () => {
  ...
  it("rejects episodes that do not belong to the user", async () => {
    ...
    const library = await createMediaLibrary({ userId: otherUserId, mediaType: "tv", name: "TV Shows", isDefault: true });
    const title = await upsertMediaTitle({
      userId: otherUserId,
      libraryId: library.id,
      mediaType: "tv",
    ...
    await expect(updateTvEpisodeMonitoringCommand(userId, {
      episodeId: episode.id,
      monitored: true,
