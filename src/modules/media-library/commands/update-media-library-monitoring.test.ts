...
describe("updateMediaLibraryMonitoringCommand", () => {
    it("updates every user title, season, and episode without touching other users", async () => {
        ...
        const database = ensureDatabaseReady();
        const updatedTitles = database
            .select()
            .from(mediaTitles)
            .where(eq(mediaTitles.userId, userId))
            .all();
        const updatedSeason = database
            .select()
            .from(tvSeasons)
            .where(eq(tvSeasons.id, season.id))
            .get();
        const updatedEpisode = database
            .select()
            .from(tvEpisodes)
            .where(eq(tvEpisodes.seasonId, season.id))
            .get();
        const untouchedTitle = database
            .select()
            .from(mediaTitles)
            .where(eq(mediaTitles.id, otherTitle!.id))
            .get();
