...
import { removeMediaTitleCommand, RemoveMediaTitleCommandError } from "./remove-media-title";
...
describe("removeMediaTitleCommand", () => {
    ...
    it("rejects titles that do not belong to the user", async () => {
        ...
        await expect(removeMediaTitleCommand(userId, { titleId: title.id })).rejects.toThrow(
    ...
    it("preserves a title associated with a still-active downloader job", async () => {
        ...
        await expect(removeMediaTitleCommand(userId, { titleId: title.id })).rejects.toMatchObject({
            code: "active_download",
        });
    ...
    it("requires an open season plan to be cancelled before title removal", async () => {
        ...
        await expect(removeMediaTitleCommand(userId, { titleId: title.id })).resolves.toMatchObject(
            { id: title.id },
        );
    });
});
