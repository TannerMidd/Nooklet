...
function matchesEpisodeTarget(text: string, season: number, episode: number): boolean {
    const standard = new RegExp(`(?:^|[^a-z0-9])s0*${season}e0*${episode}(?!\\d)`, "i");
    const compact = new RegExp(`(?:^|[^a-z0-9])0*${season}x0*${episode}(?!\\d)`, "i");
...
function matchesSeasonTarget(text: string, season: number): boolean {
    ...
    const compactSeason = new RegExp(`(?:^|[^a-z0-9])s0*${season}(?!\\d|e\\d)`, "i");
...
function matchesCompleteSeriesTarget(text: string) {
    ...
    return /\b(complete[ ._-]?(?:series|collection)|all[ ._-]?seasons|full[ ._-]?series|series[ ._-]?pack)\b/i.test(
        text,
    );
}
