...
describe("default download path", () => {
  it("keeps one default per media type and prefers it when resolving fallback targets", async () => {
    ...
    const beforeFlag = await resolveDefaultMediaLibraryDownloadTarget(userId, { mediaType: "movie" });
    ...
    const afterFlag = await resolveDefaultMediaLibraryDownloadTarget(userId, { mediaType: "movie" });
    ...
    expect(
      allOptions.filter((option) => option.isDownloadDefault).map((option) => option.id).sort(),
    ).toEqual([movieA.id, tvA.id].sort());

    const tvTarget = await resolveDefaultMediaLibraryDownloadTarget(userId, { mediaType: "tv" });
