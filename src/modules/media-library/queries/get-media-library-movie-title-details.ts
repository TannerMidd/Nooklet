...
export async function getMediaLibraryMovieTitleDetails(
  ...
  const row = database
    ...
    .where(and(eq(mediaTitles.userId, userId), eq(mediaTitles.id, titleId), eq(mediaTitles.mediaType, "movie")))
    .get();
  ...
  const fileStats = database
    ...
    .from(mediaFiles)
    .where(and(eq(mediaFiles.userId, userId), eq(mediaFiles.titleId, row.title.id), eq(mediaFiles.mediaType, "movie")))
  ...
  return {
    ...
    lastFileModifiedAt: typeof fileStats.lastModifiedAtMs === "number" ? new Date(fileStats.lastModifiedAtMs) : null,
  };
}
