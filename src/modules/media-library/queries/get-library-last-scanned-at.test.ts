...
describe("getLibraryLastScannedAt", () => {
  it("returns the most recent path scan timestamp for the given media type", async () => {
    ...
    addPath({ libraryId: tvLibrary.id, userId, path: "F:/TV/A", lastScannedAt: new Date("2026-05-01T10:00:00Z") });
    addPath({ libraryId: tvLibrary.id, userId, path: "F:/TV/B", lastScannedAt: new Date("2026-05-04T10:00:00Z") });
    addPath({ libraryId: movieLibrary.id, userId, path: "F:/Movies/A", lastScannedAt: new Date("2026-05-09T10:00:00Z") });

    expect(await getLibraryLastScannedAt(userId, "tv")).toEqual(new Date("2026-05-04T10:00:00Z"));
    expect(await getLibraryLastScannedAt(userId, "movie")).toEqual(new Date("2026-05-09T10:00:00Z"));
  });
  ...
  it("scopes results to the requesting user", async () => {
    ...
    const otherLibrary = await createMediaLibrary({ userId: otherUserId, mediaType: "tv", name: "TV", isDefault: true });

    addPath({ libraryId: otherLibrary.id, userId: otherUserId, path: "F:/TV/Other", lastScannedAt: new Date("2026-05-01T10:00:00Z") });
