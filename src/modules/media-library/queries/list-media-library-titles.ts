...
function buildTitleFilters(
    ...
    const filters: SQL[] = [eq(mediaTitles.userId, userId), eq(mediaTitles.mediaType, mediaType)];

    if (normalizedQuery) {
        filters.push(
            sql`${mediaTitles.sortTitle} like ${`%${escapeLikePattern(normalizedQuery)}%`} escape '\\'`,
        );
    }
    ...
    if (typeof input.monitored === "boolean") {
        filters.push(eq(mediaTitles.monitored, input.monitored));
    }

    if (input.libraryId === "unassigned") {
        filters.push(isNull(mediaTitles.libraryId));
    } else if (input.libraryId) {
        filters.push(eq(mediaTitles.libraryId, input.libraryId));
    }
...
function titleOrder(input: ListMediaLibraryTitlesInput, hasMediaFile: SQL) {
    if (input.sort === "recent") {
        return [desc(mediaTitles.updatedAt), asc(mediaTitles.sortTitle), asc(mediaTitles.id)];
    }

    if (input.sort === "year") {
        return [desc(mediaTitles.year), asc(mediaTitles.sortTitle), asc(mediaTitles.id)];
    }

    if (input.sort === "status") {
        return [
            asc(
                sql`case when ${hasMediaFile} then 0 when ${mediaTitles.status} = 'requested' then 1 else 2 end`,
            ),
...
export async function listMediaLibraryTitles(
        ...
        titleIds.length === 0
            ...
            : database
                  .select({
                      ...
                      qualityLabels: sql<
                          string | null
                      >`group_concat(distinct ${mediaFiles.qualityLabel})`,
                  ...
                  .all();
    const fileStatsByTitle = new Map<
        string,
        { count: number; qualities: string[]; lastModifiedAt: Date | null }
    >();
    ...
    for (const stats of fileStats) {
        fileStatsByTitle.set(stats.titleId, {
            ...
            lastModifiedAt:
                typeof stats.lastModifiedAtMs === "number"
                    ? new Date(stats.lastModifiedAtMs)
                    : null,
        });
    }

    const summaries = titles.map(({ title, library }) => {
        const stats = fileStatsByTitle.get(title.id);
        const fileCount = stats?.count ?? 0;
        const status: MediaTitleStatus =
            fileCount > 0 ? "available" : title.status === "requested" ? "requested" : "missing";
