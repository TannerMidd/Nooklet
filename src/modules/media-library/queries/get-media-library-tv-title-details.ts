...
export async function getMediaLibraryTvTitleDetails(
    ...
    const fileStats = database
        ...
        .all();
    const fileStatsByEpisode = new Map<
        string,
        { fileCount: number; qualityLabels: string[]; lastFileModifiedAt: Date | null }
    >();
    let totalFiles = 0;
    ...
    for (const episode of episodes) {
        ...
        episodesBySeason.set(episode.seasonNumber, [
            ...(episodesBySeason.get(episode.seasonNumber) ?? []),
            summary,
        ]);
    }
    ...
    const seasonSummaries = seasons.map((season) => {
        ...
        return {
            ...
            availableEpisodeCount: seasonEpisodes.filter(
                (episode) => episode.hasFile || episode.fileCount > 0,
            ).length,
    ...
    return {
        ...
        totals: {
            ...
            availableEpisodes: episodes.filter(
                (episode) => episode.hasFile || fileStatsByEpisode.has(episode.id),
            ).length,
            files: totalFiles,
        },
