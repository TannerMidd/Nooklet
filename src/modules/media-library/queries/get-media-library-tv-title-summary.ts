...
export async function getMediaLibraryTvTitleSummary(
  ...
  const countsBySeason = new Map<number, { episodeCount: number; availableEpisodeCount: number }>();

  for (const episode of episodeRows) {
    const bucket = countsBySeason.get(episode.seasonNumber) ?? { episodeCount: 0, availableEpisodeCount: 0 };
  ...
  return {
    ...
    totals: {
      ...
      availableEpisodes: seasonOverviews.reduce((acc, season) => acc + season.availableEpisodeCount, 0),
      files: totalFiles,
    },
