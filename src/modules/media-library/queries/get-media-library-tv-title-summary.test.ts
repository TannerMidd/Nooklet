...
describe("getMediaLibraryTvTitleSummary", () => {
  it("returns title metadata and per-season counts without episode arrays", async () => {
    ...
    const seasonOne = await createTvSeason({ titleId: title.id, seasonNumber: 1, title: "Season 1" });
    const seasonTwo = await createTvSeason({ titleId: title.id, seasonNumber: 2, title: "Season 2" });
...
describe("getMediaLibraryTvSeasonEpisodes", () => {
  it("returns episodes for the requested season with file stats", async () => {
    ...
    expect(seasonOneEpisodes[0]).toEqual(expect.objectContaining({
      episodeNumber: 1,
      title: "E1",
      fileCount: 1,
      qualityLabels: ["1080P"],
    ...
    expect(seasonOneEpisodes[1]).toEqual(expect.objectContaining({
      episodeNumber: 2,
      title: "E2",
      fileCount: 0,
      qualityLabels: [],
