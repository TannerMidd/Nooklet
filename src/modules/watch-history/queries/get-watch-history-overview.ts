...
export async function getWatchHistoryOverview(userId: string) {
    ...
    const latestSyncBySourceId = new Map(syncRuns.map((run) => [run.sourceId, run]));
    ...
    return {
        ...
        sources: sources.map((source) => {
            ...
            return {
                ...
                selectedUserName:
                    selectedUserMetadata?.selectedUserName ?? traktMetadata?.displayName ?? null,
                importLimit:
                    selectedUserMetadata?.importLimit ?? traktMetadata?.importLimit ?? null,
