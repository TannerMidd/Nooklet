...
function buildInput(
    overrides: Partial<{ mediaType: "tv" | "movie"; plexUserId: string; importLimit: number }> = {},
) {
    return {
...
describe("syncPlexWatchHistory", () => {
    ...
    it("fails with field=plexUserId when the requested user is not in availableUsers", async () => {
        ...
        const result = await syncPlexWatchHistory(
            USER_ID,
            buildInput({ plexUserId: "ghost-user" }),
        );
