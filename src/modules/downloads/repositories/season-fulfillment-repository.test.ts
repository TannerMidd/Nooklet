...
function seedUserWithTv(title = "Severance") {
  ...
  database.insert(mediaLibraries).values({
    id: libraryId,
    userId,
    mediaType: "tv",
    name: "TV",
  }).run();
  database.insert(mediaLibraryPaths).values({
    id: libraryPathId,
    libraryId,
    userId,
    path: `F:/TV/${userId}`,
  ...
  database.insert(mediaTitles).values({
    id: mediaTitleId,
    userId,
    libraryId,
    mediaType: "tv",
...
function seedSearchResult(userId: string, title: string, guid: string) {
  ...
  database.insert(indexerSearchRuns).values({
    id: searchRunId,
    userId,
    mediaType: "tv",
    query: title,
  ...
  database.insert(indexerSearchResults).values({
    id: resultId,
    searchRunId,
    userId,
    mediaType: "tv",
...
describe("season-fulfillment-repository", () => {
  it("converges on one open season fulfillment and enforces tenant ownership", async () => {
    ...
    expect(await findOpenSeasonFulfillment({
      userId: other.userId,
      mediaTitleId: owner.mediaTitleId,
      seasonId: owner.seasonId,
    })).toBeNull();
    await expect(createOrGetOpenSeasonFulfillment({
      userId: other.userId,
      mediaTitleId: owner.mediaTitleId,
      seasonId: owner.seasonId,
  ...
  it("tracks exact pack and per-episode attempts, exclusions, and active work", async () => {
    ...
    const secondEpisodeResultId = seedSearchResult(owner.userId, "Severance S01E02", "e02-guid");
    ...
    expect(await attachDownloadRequestToFulfillment({
      userId: owner.userId,
    ...
    expect(await findActiveDownloadRequestForFulfillment({
      userId: owner.userId,
      fulfillmentId: fulfillment.id,
      attemptStrategy: "season_pack",
  ...
  it("updates due fulfillment and episode state without crossing tenants or seasons", async () => {
    ...
    expect(await listDueDownloadFulfillments({
      userId: owner.userId,
    ...
    expect(await listDownloadFulfillmentEpisodesForIds({
      userId: owner.userId,
    ...
    await expect(upsertDownloadFulfillmentEpisode({
      userId: owner.userId,
      fulfillmentId: fulfillment.id,
  ...
  it("compares cancellation checkpoints exactly when a guarded transition resumes work", async () => {
    ...
    expect(await updateDownloadFulfillment({
      userId: owner.userId,
      fulfillmentId: fulfillment.id,
  ...
  it("lists due cancellation checkpoints independently from normal recovery work", async () => {
    ...
    expect(await listDueCancellationDownloadFulfillments({
