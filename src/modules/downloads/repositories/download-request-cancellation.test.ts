...
describe("download request cancellation persistence", () => {
  it("checkpoints before cleanup and finalizes request plus queue state with an exact CAS", async () => {
    ...
    await expect(deferDownloadRequestCancellation({
      userId,
      requestId: request.id,
      requestedAt,
    ...
    expect(await listPendingDownloadRequestCancellations(
      100,
      new Date(retryDueAt.getTime() - 1),
    )).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ id: request.id })]),
    );
    expect(await listPendingDownloadRequestCancellations(
    ...
    await expect(finalizeDownloadRequestCancellation({
      userId,
      requestId: request.id,
