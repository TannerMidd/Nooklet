...
function seedTitleAndEpisode(userId: string, keySuffix = "") {
    ...
    database
        ...
        .values({
            ...
            userId,
            mediaType: "movie",
            name: `Movies ${movieLibraryId}`,
        })
        .run();
    database
        .insert(mediaLibraries)
        .values({ id: tvLibraryId, userId, mediaType: "tv", name: `TV ${tvLibraryId}` })
        .run();
    database
        .insert(mediaTitles)
        .values({
...
describe("download-repository", () => {
    ...
    it("publishes a pending request and all downloader queue ids atomically", async () => {
        ...
        expect(submitted.queueItems.map((item) => item.externalQueueId).sort()).toEqual([
            "engine-1",
            "engine-2",
        ]);
        await expect(
    ...
    it("persists a download client, request, and queue item", async () => {
        ...
        if (!queuedRequest) {
            throw new Error("queued request missing");
        }
        ...
        const reloadedClientByConnection = await findDownloadClientByServiceConnectionId(
            userId,
            serviceConnectionId,
        );
    ...
    it("lists active and failed local-import requests for one download client", async () => {
        ...
        expect(activeRequests.map((entry) => entry.queueItem.id)).toContain(
            failedImportQueueItem.id,
        );
    ...
    it("finds the active download request for one movie or episode", async () => {
        ...
        const movieRequest = await findActiveDownloadRequestForItem({
            userId,
            mediaTitleId: movieTitleId,
        });
        const episodeRequest = await findActiveDownloadRequestForItem({
            userId,
            mediaTitleId: episodeTitleId,
            episodeId,
        });
    ...
    it("lists attempted release exclusions for a movie or episode", async () => {
        ...
        const movieExclusions = await listDownloadRequestReleaseExclusionsForItem({
            userId,
            mediaTitleId: movieTitleId,
        });
        ...
        expect(movieExclusions.resultIds).toEqual(expect.arrayContaining([secondMovieResultId]));
        expect(movieExclusions.resultIds).toHaveLength(2);
        expect(movieExclusions.releaseKeys).toEqual(
        ...
        expect(episodeExclusions.releaseKeys).toEqual(
            expect.arrayContaining([
    ...
    it("counts only budget-consuming release attempts for auto-retry", async () => {
        ...
        const seedAttempt = async (options: {
            ...
            engine: {
                ...
                failureKind: "content" | "infrastructure" | null;
    ...
    it("persists a download import run and imported files", async () => {
        ...
        if (!completedRun) {
            throw new Error("completed import run missing");
        }
    ...
    it("lists active and terminal queue attempts for season cancellation cleanup", async () => {
        ...
        expect(entries.map((entry) => entry.queueItem.externalQueueId)).toEqual(
            expect.arrayContaining(["season-attempt-1", "season-attempt-2", "season-attempt-3"]),
        );
        expect(entries.map((entry) => entry.request.status)).toEqual(
            expect.arrayContaining(["failed", "succeeded", "queued"]),
        );
    ...
    it("tracks submission time, missing-tick count, and retry count", async () => {
        ...
        if (!submitted) {
            throw new Error("submitted request missing");
        }
    ...
    it("uses stable kind and id tie-breaks for logical Activity items", async () => {
        ...
        const logicalIds = (rows: typeof first) =>
            rows.map((row) => row.fulfillment?.id ?? row.request?.id);
