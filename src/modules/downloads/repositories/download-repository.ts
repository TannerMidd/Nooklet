...
export async function findDownloadClientByServiceConnectionId(
    userId: string,
    serviceConnectionId: string,
) {
    const database = ensureDatabaseReady();
...
export async function createDownloadRequest(input: {
    ...
    return database.select().from(downloadRequests).where(eq(downloadRequests.id, id)).get()!;
}

export async function expireStalePendingDownloadReservations(
    input: {
        now?: Date;
        staleAfterMs?: number;
        userId?: string;
    ...
    if (input.userId) {
        filters.push(eq(downloadRequests.userId, input.userId));
    }
 ...
 */
export async function discardPendingDownloadRequest(input: { userId: string; requestId: string }) {
    const database = ensureDatabaseReady();
    const result = database
...
export async function recordDownloadQueueItem(input: {
    ...
    return database.select().from(downloadQueueItems).where(eq(downloadQueueItems.id, id)).get()!;
}
    ...
    database
        ...
        .where(
            and(
                ...
                eq(downloadQueueItems.id, input.queueItemId),
            ),
        )
        .run();
 ...
 */
export async function listActiveRequestsForExternalQueueId(
    userId: string,
    externalQueueId: string,
) {
    const database = ensureDatabaseReady();
...
export async function checkpointDownloadRequestCancellationForTitleRetirement(input: {
    ...
    return database.transaction((transaction) => {
        ...
        if (!current.fulfillmentId && current.cancellationRequestedAt) {
            return current;
        }
        ...
        const updated = transaction
            ...
            .set({
                ...
                statusMessage:
                    "Title removal is pending while Nooklet verifies downloader cleanup.",
...
export async function listPendingDownloadRequestCancellations(limit = 100, now = new Date()) {
    const database = ensureDatabaseReady();
    const retryCutoff = new Date(now.getTime() - DOWNLOAD_REQUEST_CANCELLATION_RETRY_DELAY_MS);
...
export async function listDownloadQueueItemsForRequest(userId: string, requestId: string) {
    const database = ensureDatabaseReady();
    ...
    return database
        ...
        .from(downloadQueueItems)
        .where(
            and(eq(downloadQueueItems.userId, userId), eq(downloadQueueItems.requestId, requestId)),
        )
...
export async function deferDownloadRequestCancellation(input: {
    ...
    const database = ensureDatabaseReady();
    const deferredAt = new Date(Math.max(Date.now(), input.requestedAt.getTime() + 1));
    const updated = database
        .update(downloadRequests)
        .set({
...
export async function finalizeDownloadRequestCancellation(input: {
        ...
        transaction
            ...
            .where(
                and(
                    ...
                    inArray(downloadQueueItems.status, [
                        "queued",
                        "downloading",
                        "paused",
                        "completed",
    ...
    return changed ? findDownloadRequestById(input.userId, input.requestId) : null;
}
 ...
 */
export async function listDownloadRequestsForFulfillment(userId: string, fulfillmentId: string) {
    const database = ensureDatabaseReady();
...
function downloadActivityHistoryKind(statuses?: DownloadRequestStatus[]) {
    ...
    if (
        statuses.every((status) =>
            activeDownloadRequestStatuses.includes(
                status as (typeof activeDownloadRequestStatuses)[number],
            ),
        )
    ) {
...
function standaloneDownloadRequestHistoryFilters(input: {
    ...
    if (input.statuses?.length) {
        filters.push(inArray(downloadRequests.status, input.statuses));
    }
...
function downloadFulfillmentHistoryFilters(
    ...
    if (historyKind === "active") {
        ...
        filters.push(
            inArray(downloadFulfillments.status, [...attentionDownloadFulfillmentStatuses]),
        );
...
function compareActivityRows(
    ...
    const groupOrder =
        (logicalOrder.get(leftLogicalId) ?? Number.MAX_SAFE_INTEGER) -
    ...
    const requestOrder =
        (right.request?.createdAt.getTime() ?? 0) - (left.request?.createdAt.getTime() ?? 0);
    ...
    if (requestIdOrder !== 0) {
        return requestIdOrder;
    }

    const queueOrder =
        (right.queueItem?.updatedAt.getTime() ?? 0) - (left.queueItem?.updatedAt.getTime() ?? 0);
    ...
    if (requestIds.length > 0) {
        rows.push(
            ...database
                ...
                .leftJoin(
                    downloadFulfillments,
                    eq(downloadFulfillments.id, downloadRequests.fulfillmentId),
                )
                .where(
                    and(
                        eq(downloadRequests.userId, userId),
...
export async function listDownloadRequestHistoryPage(input: {
    ...
    const selectedItems = database
        ...
        .offset(input.offset)
        .all() as DownloadActivityLogicalItem[];
    const total = database.select({ value: count() }).from(logicalItems).get()?.value ?? 0;
...
export async function countDownloadRequestHistory(input: {
    ...
    return database.select({ value: count() }).from(logicalItems).get()?.value ?? 0;
}
...
export async function listDownloadRequestReleaseExclusionsForItem(input: {
    ...
    const rows = database
        ...
        .leftJoin(
            indexerSearchResults,
            eq(indexerSearchResults.id, downloadRequests.searchResultId),
        )
        .where(
            and(
                eq(downloadRequests.userId, input.userId),
...
export async function completeDownloadImportRun(
    ...
    database
        ...
        .where(
            and(
                eq(downloadImportRuns.userId, input.userId),
                eq(downloadImportRuns.id, input.importRunId),
            ),
        )
        .run();
