...
import { and, asc, desc, eq, inArray, isNotNull, isNull, lte, or, type SQL } from "drizzle-orm";
...
export async function createOrGetOpenSeasonFulfillment(input: {
    ...
    if (input.targetLibraryPathId && !isOwnedLibraryPath(input.userId, input.targetLibraryPathId)) {
...
export async function updateDownloadFulfillment(input: {
    ...
    if ("expectedCancellationRequestedAt" in input) {
        ...
        filters.push(
            input.expectedCancellationRequestedAt === null
...
export async function listDueDownloadFulfillments(
    input: {
        userId?: string;
        now?: Date;
        limit?: number;
    ...
    if (input.userId) {
        filters.push(eq(downloadFulfillments.userId, input.userId));
    }
...
export async function listDueCancellationDownloadFulfillments(
    input: {
        now?: Date;
        limit?: number;
    } = {},
...
export async function upsertDownloadFulfillmentEpisode(input: {
    ...
    if (input.attemptCount !== undefined) {
        update.attemptCount = input.attemptCount;
    }
 ...
 */
export async function countDownloadFulfillmentAttempts(
    input: {
        userId: string;
        fulfillmentId: string;
    } & FulfillmentAttemptScope,
...
export async function findActiveDownloadRequestForFulfillment(
    input: {
        userId: string;
        fulfillmentId: string;
    } & FulfillmentAttemptScope,
