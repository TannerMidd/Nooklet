...
describe("conservative completed-download organization policy", () => {
    it("retains every movie disc part, skips samples, and carries only matched companions", async () => {
        ...
        const files = await Promise.all(
            [...contents.keys()].map((relativePath) => inspectedFile(sourceRoot, relativePath)),
        );
        ...
        expect(importedFiles).toEqual(
            expect.arrayContaining([
                expect.objectContaining({
                    destinationPath: path.join(destinationRoot, "Dune (1984) - Part 1.mkv"),
                }),
                expect.objectContaining({
                    destinationPath: path.join(destinationRoot, "Dune (1984) - Part 2.mkv"),
                }),
                expect.objectContaining({
                    destinationPath: path.join(
                        destinationRoot,
                        "Dune (1984) - Part 1.en.forced.srt",
                    ),
                }),
                expect.objectContaining({
                    destinationPath: path.join(destinationRoot, "Dune (1984) - Part 2.srt"),
                }),
                expect.objectContaining({
                    destinationPath: path.join(destinationRoot, "movie.nfo"),
                }),
                expect.objectContaining({
                    destinationPath: path.join(destinationRoot, "poster.jpg"),
                }),
            ]),
        );
        expect(importedFiles).toHaveLength(6);
        await expect(
            readFile(path.join(destinationRoot, "Dune (1984) - Part 1.mkv"), "utf8"),
        ).resolves.toBe("first half of the movie");
        await expect(
            readFile(path.join(destinationRoot, "Dune (1984) - Part 2.mkv"), "utf8"),
        ).resolves.toBe("second half of the movie");
        await expect(
            readFile(path.join(sourceRoot, "Samples", "Dune.Sample.mkv"), "utf8"),
        ).resolves.toBe("sample clip");
        await expect(readFile(path.join(sourceRoot, "English.srt"), "utf8")).resolves.toBe(
            "ambiguous subtitle",
        );
    });
    ...
    it("keeps an episode-shaped extra outside the episode mapping", async () => {
        ...
        const files = await Promise.all(
            ["Show.S01E01.mkv", "Show.S01E01.en.srt", "Extras/Show.S01E99.mkv"].map(
                (relativePath) => inspectedFile(sourceRoot, relativePath),
            ),
        );
        ...
        const [organized] = await organizeCompletedDownloadFiles([
            readyDownload({
                ...
                year: 2026,
                titleEpisodes: [
                    { id: "episode-1", seasonNumber: 1, episodeNumber: 1, title: "Pilot" },
                ],
                files,
            }),
        ]);
        ...
        expect(organized.kind === "organized" ? organized.files : []).toEqual(
            expect.arrayContaining([
                expect.objectContaining({
                    destinationPath: path.join(
                        titleRoot,
                        "Season 01",
                        "Show (2026) - S01E01 - Pilot.mkv",
                    ),
    ...
    it("detects a companion destination collision before moving any source file", async () => {
        ...
        const files = await Promise.all(
            ["Movie.mkv", "Movie.en.srt", "Subs/Movie.en.srt"].map((relativePath) =>
                inspectedFile(sourceRoot, relativePath),
            ),
        );
        ...
        await expect(readFile(path.join(sourceRoot, "Movie.mkv"), "utf8")).resolves.toBe(
            "movie bytes",
        );
        await expect(readFile(path.join(sourceRoot, "Movie.en.srt"), "utf8")).resolves.toBe(
            "first subtitle",
        );
        await expect(readFile(path.join(sourceRoot, "Subs", "Movie.en.srt"), "utf8")).resolves.toBe(
            "second subtitle",
        );
        await expect(
            access(path.join(targetRoot, "Movie (2026)", "Movie (2026).mkv")),
