...
import {
    type ResolvedCompletedDownload,
    type ImportableCompletedDownload,
...
export function isRetryableCompletedMediaFailure(message: string) {
    return message === noMediaFilesFoundMessage || message === noPrimaryMediaFilesFoundMessage;
}
...
async function walkImportFiles(
    rootPath: string,
    currentPath: string,
): Promise<InspectedDownloadFile[]> {
...
async function inspectImportableDownload(
    source: ImportableCompletedDownload,
        ...
        return {
            ...
            message:
                error instanceof Error
                    ? error.message
                    : "Completed download files could not be read.",
        };
    }
}
