...
function retryableFailureMatch(
    download: OrganizedCompletedDownload,
): MatchedCompletedDownload | null {
...
export async function retryFailedCompletedDownloads(
    ...
    for (const download of downloads) {
        ...
        if (!fulfillment || reconciledFulfillmentIds.has(fulfillment.id)) {
            continue;
        }
    ...
    for (const download of downloads) {
        ...
        if (
            !mediaTitleId ||
            !itemKey ||
            retriedItemKeys.has(itemKey) ||
            successfulItemKeys.has(itemKey)
        ) {
            continue;
        }
