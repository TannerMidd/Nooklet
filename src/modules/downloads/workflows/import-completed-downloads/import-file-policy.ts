...
export const importVideoExtensions = new Set([
  ".avi", ".m4v", ".mkv", ".mov", ".mp4", ".mpeg", ".mpg", ".ts", ".wmv",
]);
...
export const noPrimaryMediaFilesFoundMessage = "The completed download contained only samples or extras, not a primary movie or episode file.";
...
export function classifyVideoImportRole(
  ...
  if (hasLargerSibling(file, videos)) {
    ...
    if (
      /^(?:behind[ ._-]+the[ ._-]+scenes?|deleted[ ._-]+scenes?|extras?|featurettes?|interviews?|trailers?)(?:$|[ ._-])/i.test(stem)
    ) {
      return "extra";
    }
  }
...
export function moviePartNumber(relativePath: string): number | null {
  ...
  const match = stem.match(/(?:^|[ ._()\[\]-])(?:cd|disc|disk|part|pt)[ ._-]?(\d{1,2})(?:$|[ ._()\[\]-])/i);
...
export function matchedCompanionSuffix(
  ...
  if (companionKind !== "subtitle" || !companionStem.toLowerCase().startsWith(videoStem.toLowerCase())) {
    return null;
  }
