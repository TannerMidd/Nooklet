...
import { noMediaFilesFoundMessage, noPrimaryMediaFilesFoundMessage } from "./file-inspection";
...
describe("retryFailedCompletedDownloads", () => {
        ...
        expect(updateRequestStatusMock).toHaveBeenCalledWith(
            expect.objectContaining({
                statusMessage: expect.stringContaining(
                    "Auto-retry stopped after 3 failed download attempts",
                ),
            }),
        );
    });
    ...
    it("keeps cycling to new releases when prior attempts were zero-transfer abandons", async () => {
        ...
        expect(result).toEqual({ attemptedCount: 1, queuedCount: 1, failedCount: 0 });
        expect(searchLibraryItemReleasesMock).toHaveBeenCalledWith(
            "user1",
            expect.objectContaining({
    ...
    it("does not retry a failed duplicate when the same item imported successfully", async () => {
        ...
        const successfulImport = {
            ...
            files: [
                {
                    sourcePath: "F:/Downloads/Star Trek.mkv",
                    destinationPath: "F:/Media/Movies/Star Trek (2009).mkv",
                },
            ],
        } as never;
