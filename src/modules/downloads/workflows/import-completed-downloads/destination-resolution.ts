...
import { listTvEpisodesForTitle, type TvEpisodeRecord } from "@/modules/media-library/public";
...
export type ResolvedCompletedDownload =
    FailedCompletedDownloadResolution | ImportableCompletedDownload;
    ...
    for (const match of matches) {
        if (match.historyItem.statusKind === "failed") {
            resolvedDownloads.push({
                ...
                match,
                message:
                    match.historyItem.failMessage ??
                    "The downloader reported that the download failed.",
            });
            continue;
        }
        ...
        const titleEpisodes =
            match.request.mediaType === "tv" &&
            !match.request.episodeId &&
