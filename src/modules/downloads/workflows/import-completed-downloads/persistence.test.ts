...
describe("persistCompletedDownloadImports", () => {
    it("records a successful import and completes the download request", async () => {
        ...
        expect(importedFiles[0]?.destinationPath).toBe(
            "F:/Media/Movies/Arrival (2016)/Arrival (2016).mkv",
        );
    });
    ...
    it("commits the durable season checkpoint with the successful request transition", async () => {
        ...
        const result = await persistCompletedDownloadImports(userId, [
            {
                kind: "organized",
                destinationRootPath: "F:/Media/TV/Checkpoint Test Show",
                files: [
                    {
                        sourcePath: "C:/Downloads/complete/Checkpoint/episode.mkv",
                        destinationPath: "F:/Media/TV/Checkpoint Test Show/Season 01/episode.mkv",
                        episodeMatch: {
                            seasonNumber: 1,
                            episodeNumber: 1,
                            episodeId: fixture.episodeId,
    ...
    it("rejects a cross-tenant episode match without partial import writes", async () => {
        ...
        const { libraryId, libraryPathId } = seedMoviePath(userId);
        const foreignEpisodeId = seedTvEpisode(foreignUserId);
        const request = await createDownloadRequest({
            userId,
            mediaType: "movie",
        ...
        await expect(
            persistCompletedDownloadImports(userId, [
                {
                    kind: "organized",
                    destinationRootPath: "F:/Media/Movies/Cross-tenant episode",
                    files: [
                        {
                            sourcePath: "C:/Downloads/complete/Cross-tenant/episode.mkv",
                            destinationPath: "F:/Media/Movies/Cross-tenant episode/episode.mkv",
                            episodeMatch: {
                                seasonNumber: 1,
                                episodeNumber: 1,
                    ...
                    source: {
                        source: {
                            sourceRootPath: "C:/Downloads/complete/Cross-tenant",
                            target: { path: { id: libraryPathId } },
                            match: {
                                request,
                                queueItem,
                                historyItem: {
                                    id: "engine-download-cross-tenant-episode",
                                    statusKind: "completed",
                                    completedAt: new Date("2026-05-07T00:00:00Z"),
                                },
                            },
        ...
        const storedQueueItem = database
            ...
            .where(eq(downloadQueueItems.id, queueItem.id))
            .get();
        const importRuns = database
            .select()
            .from(downloadImportRuns)
            .where(eq(downloadImportRuns.requestId, request.id))
            .all();
        const importedFiles = database
            .select()
            .from(downloadImportedFiles)
        ...
        expect(importedFiles).toHaveLength(0);
    ...
    it("rejects a missing episode match without terminalizing the request", async () => {
        ...
        await expect(
            persistCompletedDownloadImports(userId, [
                {
                    kind: "organized",
                    destinationRootPath: "F:/Media/Movies/Missing episode",
                    files: [
                        {
                            sourcePath: "C:/Downloads/complete/Missing/episode.mkv",
                            destinationPath: "F:/Media/Movies/Missing episode/episode.mkv",
                            episodeMatch: {
                                seasonNumber: 1,
                                episodeNumber: 1,
                    ...
                    source: {
                        source: {
                            sourceRootPath: "C:/Downloads/complete/Missing",
                            target: { path: { id: libraryPathId } },
                            match: {
                                request,
                                queueItem,
                                historyItem: {
                                    id: "engine-download-missing-episode",
                                    statusKind: "completed",
                                    completedAt: new Date("2026-05-07T00:00:00Z"),
                                },
                            },
    ...
    it("rolls back every import write when a later file write fails", async () => {
        const userId = await seedUser();
        const { libraryId, libraryPathId } = seedMoviePath(userId);
        const episodeId = seedTvEpisode(userId);
        const request = await createDownloadRequest({
            userId,
            mediaType: "movie",
            requestedTitle: "Atomicity Test Movie",
            status: "queued",
            targetLibraryId: libraryId,
            targetLibraryPathId: libraryPathId,
        });
        ...
        await expect(
            persistCompletedDownloadImports(userId, [
                {
                    kind: "organized",
                    destinationRootPath: "F:/Media/Movies/Atomicity Test Movie",
                    files: [
                        {
                            sourcePath: "C:/Downloads/complete/Atomicity/Atomicity.mkv",
                            destinationPath: "F:/Media/Movies/Atomicity Test Movie/Atomicity.mkv",
                            episodeMatch: {
                                seasonNumber: 1,
                                episodeNumber: 1,
                                episodeId,
                    ...
                    source: {
                        source: {
                            sourceRootPath: "C:/Downloads/complete/Atomicity",
                            target: { path: { id: libraryPathId } },
                            match: {
                                request,
                                queueItem,
