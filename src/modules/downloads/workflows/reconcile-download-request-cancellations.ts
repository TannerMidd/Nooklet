...
async function reconcileRequest(
    ...
    try {
        ...
        if (
            ...
            current.cancellationRequestedAt.getTime() !== request.cancellationRequestedAt?.getTime()
        ) {
        ...
        const renew = async () => {
            ...
            if (!renewed) {
                throw new Error(
                    "Download cancellation ownership expired before cleanup was verified.",
                );
            }
        ...
        if (pendingRemoval) {
            await deferDownloadRequestCancellation({
                ...
                requestedAt,
                message:
                    removal.get(pendingRemoval)?.message ??
