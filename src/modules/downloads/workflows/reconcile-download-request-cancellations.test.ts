...
beforeEach(() => {
    ...
    removeEngineMock.mockResolvedValue(
        new Map([
...
describe("reconcilePendingDownloadRequestCancellations", () => {
    it("verifies every physical engine job before finalizing a null-fulfillment pack", async () => {
        ...
        expect(listPendingMock).toHaveBeenCalledWith(DOWNLOAD_REQUEST_CANCELLATION_PASS_LIMIT);
        expect(removeEngineMock).toHaveBeenCalledWith("user-1", ["engine-1", "engine-2"], {
            beforeExternalPhase: expect.any(Function),
        });
        expect(removeEngineMock.mock.invocationCallOrder[0]).toBeLessThan(
            finalizeMock.mock.invocationCallOrder[0],
        );
