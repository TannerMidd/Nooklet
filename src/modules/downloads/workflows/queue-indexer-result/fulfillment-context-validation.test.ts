...
async function expectInvalid(
  ...
  const leasedContext = context && typeof context.fulfillmentId === "string"
...
describe("validateQueueIndexerResultFulfillmentContext", () => {
  it("accepts an absent context without touching fulfillment persistence", async () => {
    await expect(validateQueueIndexerResultFulfillmentContext(
      "user-1",
      seasonRequest,
      { fulfillmentId: null, attemptStrategy: null, attemptNumber: null },
    )).resolves.toEqual({});
  ...
  ])("rejects a season-pack context that is not the plan's exact target %#", async (
    request,
    storedFulfillment,
  ) => {
  ...
  it("accepts an exact season-pack context", async () => {
    ...
    await expect(validateQueueIndexerResultFulfillmentContext("user-1", seasonRequest, {
      fulfillmentId: "fulfillment-1",
      attemptStrategy: "season_pack",
      attemptNumber: 2,
  ...
  it.each([
    ...
    await expectInvalid({ ...seasonRequest, episodeId }, {
      fulfillmentId: "fulfillment-1",
