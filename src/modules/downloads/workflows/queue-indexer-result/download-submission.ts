...
import { detectNewznabErrorDocument, formatNewznabErrorDocument } from "@/modules/indexers/public";
...
async function submitToEngine(
    ...
    try {
        ...
        if (error instanceof EnqueueNzbDownloadError) {
            throw new QueueIndexerResultWorkflowError(
                error.code === "invalid_nzb" ? "release_unavailable" : "download_capacity_exceeded",
                error.message,
                error.capacity,
            );
        }
