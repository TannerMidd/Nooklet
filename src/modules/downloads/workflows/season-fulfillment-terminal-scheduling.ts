...
export async function scheduleSeasonFulfillmentAfterRequest(
  ...
  try {
    ...
    if (!fulfillment) return identified;
    if (fulfillment.status === "succeeded") return fulfillment;
    ...
    const status = outcome.status === "succeeded"
      ...
      : shouldRetry
        ? "retry_wait" as const
        : "blocked" as const;
    const nextAttemptAt = status === "blocked"
      ? null
      : new Date(Date.now() + recoveryCheckpointDelayMs);
    const statusMessage = outcome.status === "succeeded"
    ...
    if (!transitioned) return fulfillment;
