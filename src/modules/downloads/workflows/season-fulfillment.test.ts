...
type MockEpisodeState = {
  ...
  episodeId: string;
  status: "pending" | "active" | "retry_wait" | "unavailable" | "blocked" | "succeeded" | "deferred";
  attemptCount: number;
  nextAttemptAt: Date | null;
  statusMessage: string | null;
  createdAt: Date;
...
beforeEach(() => {
  ...
  updateFulfillmentMock.mockImplementation(async (input) => {
    if (input.userId !== fulfillment.userId || input.fulfillmentId !== fulfillment.id) return null;
    ...
    if ("expectedCancellationRequestedAt" in input) {
      ...
      if (expectedTime !== currentTime) return null;
    }
  ...
  findEpisodeMock.mockImplementation(async (_userId, episodeId) => ({
    title: { id: "title-1" },
    episode: makeEpisode(
      episodeId,
...
describe("season fulfillment recovery", () => {
  it("creates a restart-safe checkpoint before the initial pack search", async () => {
    ...
    expect(createFulfillmentMock).toHaveBeenCalledWith(expect.objectContaining({
      status: "active",
      strategy: "season_pack",
  ...
  it("falls back from no season pack and queues every eligible missing aired episode", async () => {
    ...
    expect(searchMock.mock.calls.slice(1).map(([, input, context]) => ({ input, context })))
      .toEqual(expect.arrayContaining(episodes.map((episode) => ({
        input: {
          titleId: "title-1",
  ...
  it("skips owned, future, unmonitored, and already-active episodes", async () => {
    ...
    expect(Object.fromEntries(episodeStates.map((state) => [state.episodeId, state.status])))
      .toEqual({
        owned: "succeeded",
        future: "deferred",
    ...
    expect(episodeStates.find((state) => state.episodeId === "future")?.nextAttemptAt)
      .toEqual(new Date("2026-08-01T00:00:00.000Z"));
    expect(episodeStates.find((state) => state.episodeId === "unmonitored")?.nextAttemptAt)
      .toBeNull();
  });
  ...
  it("keeps capacity-limited episode children retryable without consuming attempts", async () => {
    const episodes = [
      makeEpisode("episode-1", 1),
      makeEpisode("episode-2", 2),
    ];
    listEpisodesMock.mockResolvedValue(episodes as never);
    countAttemptsMock.mockResolvedValue(0);
    searchMock.mockImplementation(async (_userId, input) => (
    ...
    expect(episodeStates).toEqual(expect.arrayContaining([
      expect.objectContaining({
        episodeId: "episode-1",
        status: "active",
      }),
  ...
  it("does not persist a due-worker error after another owner takes the lease", async () => {
    ...
    acquireMock
      .mockResolvedValueOnce(attemptLease)
      .mockResolvedValueOnce(null);

    const result = await runDueSeasonFulfillments();

    expect(result).toEqual({ attemptedCount: 1, queuedCount: 0, failedCount: 1 });
    expect(updateFulfillmentMock).not.toHaveBeenCalledWith(expect.objectContaining({
      status: "retry_wait",
  ...
  it("does not downgrade an episode imported while an unexpected search error unwinds", async () => {
    ...
    expect(upsertEpisodeMock).not.toHaveBeenCalledWith(expect.objectContaining({
      episodeId: "episode-1",
      status: "retry_wait",
    }));
  });
  ...
  it("resumes a due episode plan without queueing the same episode twice", async () => {
    ...
    searchMock.mockImplementation(async (_userId, input) => {
      if (input.episodeId) activeEpisodeIds.add(input.episodeId);
      return queuedSearchResult();
    });
