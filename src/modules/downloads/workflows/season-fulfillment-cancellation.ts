...
const cancellableStatuses = ["active", "retry_wait", "partial", "blocked", "failed"] as const;
...
async function checkpointFulfillmentCancellation(
    ...
    if (!cancellableStatuses.includes(fulfillment.status as (typeof cancellableStatuses)[number])) {
        return null;
    }
