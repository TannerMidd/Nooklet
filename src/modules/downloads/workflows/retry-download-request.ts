...
export type RetryDownloadRequestResult = {
    queued: boolean;
    reason:
        "queued" | "episode_fallback" | "search_failed" | "no_matching_release" | "queue_failed";
    message: string | null;
};
...
export async function resumeSeasonFulfillmentWorkflow(
    ...
    const resumed =
        ...
        (openFulfillmentStatuses.includes(updated.status) || completed);
    ...
    if (request.fulfillmentId && request.episodeId) {
        ...
        if (!episode) {
            return {
                ...
                message: "That episode is no longer available in the library.",
            };
        }
        ...
        return {
            ...
            message: queued
                ? null
                : "Immediate alternatives are exhausted; Nooklet will search again automatically.",
        };
    }
