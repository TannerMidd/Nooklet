...
describe("scheduleSeasonFulfillmentAfterRequest", () => {
    ...
    it("uses the same restart checkpoint for an episode child", async () => {
        ...
        expect(upsertEpisodeMock).toHaveBeenCalledWith(
            expect.objectContaining({
                episodeId: "episode-1",
                status: "retry_wait",
                nextAttemptAt: new Date("2026-07-15T18:05:00.000Z"),
    ...
    it("acquires the lease before re-reading and mutating terminal state", async () => {
        ...
        expect(acquireWorkMock.mock.invocationCallOrder[0]).toBeLessThan(
            findFulfillmentMock.mock.invocationCallOrder[0] ?? 0,
        );
        expect(findFulfillmentMock.mock.invocationCallOrder[0]).toBeLessThan(
            updateMock.mock.invocationCallOrder[0] ?? 0,
        );
    });
