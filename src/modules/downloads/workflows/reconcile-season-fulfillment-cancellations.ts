...
export async function reconcileSeasonFulfillmentCancellation(
    ...
    try {
        ...
        const pending = entries.filter(
            (entry) => checks.get(entry.queueItem.externalQueueId)?.removed !== true,
        );
        ...
        if (stillOwned) {
            ...
            const current = await findDownloadFulfillmentById(userId, fulfillmentId).catch(
                () => null,
            );
...
export async function reconcilePendingSeasonFulfillmentCancellations() {
    ...
    for (const fulfillment of due) {
        ...
        if (result === "cancelled") {
            cancelledCount += 1;
        } else if (result === "pending" || result === "busy") {
            pendingCount += 1;
