...
function aggregateQueueItems(items: NonNullable<DownloadActivityRow["queueItem"]>[]) {
    ...
    const unique = [
        ...new Map(items.map((item, index) => [item.id ?? `queue-${index}`, item])).values(),
    ];
    const status = [...unique].sort(
        (left, right) =>
    ...
    const progressPercent =
        unique.reduce((sum, item) => sum + item.progressPercent, 0) / unique.length;
...
function mapDownloadActivityRows(rows: DownloadActivityRow[]) {
    ...
    return [...groups.entries()].map(([logicalId, group]) => {
        ...
        for (const row of group) {
            ...
            const attempt = attemptsById.get(row.request.id) ?? {
                request: row.request,
                queueItems: [],
            };
            ...
            if (
                row.queueItem &&
                !attempt.queueItems.some((item) => item.id === row.queueItem!.id)
            ) {
        ...
        const fulfillmentNeedsAttention = fulfillment
            ...
            : false;
        const canRetry =
            fulfillmentCancellationPending ||
        ...
        const retryAction: DownloadActivityEntry["retryAction"] = canRetry
            ...
            : null;
        const activeAttempts = attempts.filter(({ request }) =>
            activeRequestStatuses.includes(request.status),
        );
        const queueItems = (
            activeAttempts.length > 0 ? activeAttempts : representative ? [representative] : []
        ...
        return {
            ...
            status,
            statusMessage:
                fulfillment?.statusMessage ?? representative?.request.statusMessage ?? null,
            technicalStatusMessage:
                representative?.request.statusMessage ?? fulfillment?.statusMessage ?? null,
...
async function addSeasonEpisodeProgress(userId: string, entries: DownloadActivityEntry[]) {
...
export async function getDownloadActivityPage(input: {
    ...
    const [active, attention, completed] = await Promise.all(
        (Object.keys(downloadActivityViews) as DownloadActivityView[]).map((view) =>
            countDownloadRequestHistory({
                userId: input.userId,
