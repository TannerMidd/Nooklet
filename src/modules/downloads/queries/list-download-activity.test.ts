...
function bareFulfillmentRow(
  overrides: Partial<ReturnType<typeof openEpisodeFulfillment>> = {},
) {
  return {
    request: null,
    queueItem: null,
...
describe("listDownloadActivity", () => {
  ...
  it("groups a recovering season and does not expose failed pack attempts as terminal", async () => {
    ...
    expect(result[0]).toEqual(expect.objectContaining({
      ...
      planMessage: "Season plan switched to individual episodes after 3 season pack attempts.",
      nextAttemptAt: new Date("2026-07-15T14:00:00Z"),
    }));
  });
  ...
  it("shows durable cancellation as in progress with an undo action", async () => {
    listRequestsMock.mockResolvedValue([bareFulfillmentRow({
      cancellationRequestedAt: new Date("2026-07-15T13:30:00Z"),
      statusMessage: "Cancellation is pending while Nooklet verifies the downloader queue.",
    } as never)] as never);
  ...
  it("surfaces a mixed blocked and unavailable episode plan as resumable attention", async () => {
    ...
    expect(entry).toEqual(expect.objectContaining({
      ...
      canRetry: true,
      retryAction: "resume_season_recovery",
      planMessage: "Season recovery paused after 3 season pack attempts. Resume it to re-check every missing episode.",
    }));
  });
