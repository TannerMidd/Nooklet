...
export function SearchableSelect({
  ...
  const highlightedOptionId = totalRows > 0
    ? `${listboxId}-option-${effectiveHighlight}`
    : undefined;
    ...
    <div ref={containerRef} className={cn("relative", className)}>
      ...
      {open ? (
        <div className="absolute left-0 right-0 z-30 mt-2 overflow-hidden rounded-xl border border-cream/10 bg-panel shadow-[0_24px_48px_-32px_rgba(0,0,0,0.9)]">
          ...
          <ul id={listboxId} role="listbox" aria-label={`${ariaLabel} options`} className="max-h-64 overflow-y-auto py-1">
            ...
            {filtered.map((option, index) => {
              ...
              return (
                ...
                >
                  ...
                  {isSelected ? <span aria-hidden="true" className="text-accent">✓</span> : null}
                </li>
              );
            })}
            {showCustomRow ? (
