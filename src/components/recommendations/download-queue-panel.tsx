...
export function DownloadQueuePanel({ initialState, className }: DownloadQueuePanelProps) {
  ...
  const executeQueueAction = useCallback(async (action: DownloadQueueActionInput) => {
    setActionError(null);
    setIsMutating(true);
    ...
    try {
      ...
      if (!response.ok) {
        setActionError(getActionErrorMessage(payload) ?? "Unable to update the download queue right now.");
        return;
      }
  ...
  return (
    <div className={cn("space-y-7", className)}>
      ...
      <AlertDialog
        ...
        description={<>Removing <strong className="text-foreground">{pendingRemoval?.title}</strong> permanently deletes its working and completed files. If it belongs to a season recovery plan, the whole plan is cancelled and every linked sibling job and its files are also deleted. This cannot be undone.</>}
        ...
        <section className="flex flex-col gap-5 rounded-3xl border border-cream/[0.08] bg-[linear-gradient(125deg,rgba(232,165,80,0.08),transparent_55%),rgba(255,244,230,0.03)] px-6 py-[22px]">
          <div className="flex flex-wrap items-start justify-between gap-x-7 gap-y-4">
            <div className="min-w-0">
              <p className="inline-flex items-center gap-2.5 font-heading text-3xl leading-[1.1] text-foreground">
                <span aria-hidden="true" className={cn("h-[9px] w-[9px] shrink-0 rounded-full bg-accent shadow-[0_0_14px_rgba(232,165,80,0.55)]", liveQueue.isLive ? "nk-pulse" : null)} />
                {liveQueue.status}
              </p>
            ...
            <div className="flex shrink-0 gap-8">
              {liveQueue.figures.map((figure) => (
                <div key={figure.label}>
                  ...
                  <p className="mt-[7px] text-[11px] font-semibold uppercase tracking-[0.09em] text-muted">{figure.label}</p>
                </div>
              ))}
            </div>
          </div>
          {liveQueue.progress ? (
            <div className="flex flex-col gap-2.5">
              <div role="progressbar" aria-label="Overall queue progress" aria-valuemin={0} aria-valuemax={100} aria-valuenow={liveQueue.progress.percent} className="h-1.5 overflow-hidden rounded-full bg-cream/[0.07]">
                <div className="h-full rounded-full bg-accent transition-[width] duration-500" style={{ width: `${liveQueue.progress.percent}%` }} />
              </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Button variant="secondary" size="sm" disabled={busy} onClick={() => void queueContext.refreshQueue()}>
      ...
      {queueState.connectionStatus !== "verified" || !snapshot ? (
        ...
        <section className="space-y-4 rounded-2xl border border-cream/[0.08] bg-cream/[0.02] p-4 sm:p-5">
          <div className="flex items-center justify-between gap-3">
            <h3 className="font-heading text-[21px] text-foreground">Built-in downloader</h3>
            <Button
              variant="secondary"
              size="sm"
              disabled={busy || snapshot.totalQueueCount === 0}
              onClick={() => void submitQueueAction({ type: snapshot.paused ? "resumeQueue" : "pauseQueue" })}
            >
          ...
          {snapshot.items.length === 0 ? (
            <div className="rounded-xl border border-dashed border-cream/[0.12] bg-cream/[0.02] px-5 py-4 text-sm text-muted">No active downloads in this queue.</div>
          ) : (
            <div className="max-h-[52vh] space-y-2.5 overflow-y-auto pr-1">
              {snapshot.items.map((item, index) => {
                ...
                return (
                  <article key={item.id} className="flex flex-col gap-3 rounded-xl border border-cream/[0.08] bg-cream/[0.03] px-4 py-3">
                    <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                      <div className="min-w-0">
                        <p className="truncate text-sm font-semibold text-foreground" title={item.title}>{item.title}</p>
                        <p className="mt-0.5 text-xs text-muted">{[item.category, item.priority ? `${item.priority} priority` : null, item.sizeLabel, ...item.labels].filter(Boolean).join(" · ") || item.status}</p>
                      </div>
                      <div className="flex shrink-0 flex-wrap items-center gap-2">
                        <span className="mr-1 text-[13px] font-semibold text-accent">{isPaused ? "Paused" : formatProgressPercent(item.progressPercent)}</span>
                        <Button variant="secondary" size="sm" disabled={busy || index === 0} onClick={() => void submitQueueAction({ type: "move", itemId: item.id, direction: "up" })} aria-label={`Move ${item.title} up`}>Up</Button>
                        <Button variant="secondary" size="sm" disabled={busy || index >= snapshot.items.length - 1} onClick={() => void submitQueueAction({ type: "move", itemId: item.id, direction: "down" })} aria-label={`Move ${item.title} down`}>Down</Button>
                        {canPauseItem(item.status) ? (
                          <Button variant="secondary" size="sm" disabled={busy} onClick={() => void submitQueueAction({ type: actionType, itemId: item.id })}>
                            {pendingActionKey === `${actionType}:${item.id}` ? isPaused ? "Resuming..." : "Pausing..." : isPaused ? "Resume" : "Pause"}
                          </Button>
                        ) : <span className="px-2 text-xs text-muted">{item.status}</span>}
                        <Button variant="danger" size="sm" disabled={busy} onClick={() => void submitQueueAction({ type: "remove", itemId: item.id }, item.title)}>
