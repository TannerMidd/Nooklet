...
export function RecommendationDownloadStatus({
  ...
  const details = [
    matchedItem.timeLeft ? `${matchedItem.timeLeft} left` : null,
    matchedItem.sizeLeftLabel,
  ].filter(Boolean).join(" / ") || "Open In progress for queue controls.";
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="text-xs font-medium text-muted">{statusLabel} / {progressLabel}</span>
      </div>
      <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-cream/[0.07]">
        <div className="h-full rounded-full bg-accent transition-[width] duration-500" style={{ width: progressLabel }} />
      </div>
