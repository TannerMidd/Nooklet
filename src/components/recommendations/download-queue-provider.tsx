...
export function DownloadQueueProvider({ children }: { children: ReactNode }) {
  ...
  const refreshQueue = useCallback(async () => {
    ...
    try {
      ...
      if (stateVersionRef.current === startingVersion) setQueueState(nextState);
    } finally {
  ...
  useEffect(() => {
    ...
    const startingVersion = stateVersionRef.current;
    refreshInFlightRef.current = true;
    void fetchDownloadQueueState().then((nextState) => {
      if (active && stateVersionRef.current === startingVersion) setQueueState(nextState);
    }).finally(() => {
      refreshInFlightRef.current = false;
    });
    return () => {
