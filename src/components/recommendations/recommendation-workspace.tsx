...
function formatGenreSummary(selectedGenres: readonly RecommendationGenre[]) {
    return selectedGenres.length > 0 ? formatRecommendationGenres(selectedGenres).join(", ") : null;
}
...
export async function RecommendationWorkspace({
    ...
    const [connectionSummaries, recentRuns, selectedOverview, libraryOverview, pathOptions] =
        await Promise.all([
    ...
    const canRequest = Boolean(
        aiProvider && aiProvider.status !== "disconnected" && hasVerifiedTmdbForLanguage,
    );
    const recommendationRequestBlockedMessage =
        !aiProvider || aiProvider.status === "disconnected"
            ? (aiProvider?.statusMessage ??
              "Configure the AI provider before requesting recommendations.")
            : !hasVerifiedTmdbForLanguage
              ...
              : null;
    const defaultModel = preferences.defaultAiModel?.trim().length
        ? preferences.defaultAiModel
        : (aiProvider?.model ?? "gpt-4.1-mini");
    const availableModels = aiProvider?.availableModels ?? [];
    const featuredRun = recentRuns.find((run) => run.id === activeRunId) ?? recentRuns[0] ?? null;
    ...
    const featuredRunIsFresh = Boolean(
        wasJustGenerated && featuredRun && featuredRun.id === activeRunId,
    );
    const currentWorkspaceHref = buildWorkspaceHref(routePath, activeRunId, wasJustGenerated);
    const overviewForModal =
        selectedOverview?.item.mediaType === mediaType ? selectedOverview : null;

    const heroItem =
        !featuredRunIsPending && featuredRun && featuredRun.items.length > 0
            ? featuredRun.items[0]
            : null;
        ...
        <div className="nk-enter space-y-9">
            ...
            {featuredRunIsPending && featuredRun ? (
                <section className="rounded-4xl border border-cream/[0.09] bg-cream/[0.03] p-10">
                    ...
                    <p className="mt-2 max-w-xl text-[15px] leading-6 text-muted">
                        Settle in — the worker is steeping your picks. Results pour in automatically
                        as they finish.
                    </p>
                    <div className="mt-5 flex items-center gap-3 text-sm text-muted">
                        <span
                            ...
                            aria-hidden="true"
                        />
                        <RecommendationPendingTimer
                            startedAt={featuredRun.createdAt}
                    ...
                    <div className="relative max-w-[640px] space-y-3.5 p-7 sm:p-10">
                        ...
                        <div className="flex flex-wrap items-center gap-2.5 text-[13px] text-muted">
                            {heroItem.confidenceLabel ? (
                                <StatusDot
                                    tone={
                                        isHighConfidence(heroItem.confidenceLabel)
                                            ? "ok"
                                            : "neutral"
                                    }
                        ...
                        <div className="flex flex-wrap items-center gap-2.5 pt-1.5">
                            <RecommendationAddForm
                                ...
                                mediaType={heroItem.mediaType}
                                tmdbId={
                                    heroItem.providerMetadata?.tmdbDetails?.mediaType ===
                                    heroItem.mediaType
                ...
                <section className="space-y-4.5">
                    ...
                    {batchItems.length === 0 && !heroItem ? (
                        <p className="text-sm leading-6 text-muted">
                            This run finished without saved items — run it again to get a fresh
                            batch.
                        </p>
                    ) : null}
                                    ...
                                    >
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <p className="mt-0.5 text-[12.5px] text-muted">
                                                {run.itemCount} items · requested{" "}
                                                {run.requestedCount} · {run.aiModel ?? defaultModel}{" "}
                                                · temp {formatTemperature(run.aiTemperature)}
            ...
            {overviewForModal ? (
                <RecommendationTitleOverviewDialog
                    ...
                    actionReturnHref={appendDetailsParam(
                        currentWorkspaceHref,
                        overviewForModal.item.itemId,
                    )}
                    libraries={libraryOptions}
                    qualityProfiles={qualityProfiles}
                    pathOptions={pathOptions}
