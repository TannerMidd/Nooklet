        ...
        >
          <ul className="space-y-3 text-sm leading-6 text-foreground">
            {acceptanceCriteria.map((item) => (
              <li key={item} className="rounded-lg border border-cream/[0.08] bg-cream/[0.04] px-4 py-3">
                {item}
              </li>
            ))}
          </ul>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {relatedLinks.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="rounded-lg border border-cream/[0.08] bg-cream/[0.04] px-3 py-2.5 transition hover:border-accent/40 hover:bg-panel"
              >
