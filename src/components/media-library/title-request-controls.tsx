...
export function TitleRequestControls({
  ...
  const selectedLibrary = matchingLibraries.find((library) => library.id === selectedLibraryId) ?? null;
  const selectedPath = matchingPathOptions.find((option) => option.id === selectedTargetPathId) ?? null;
  ...
  return (
    <>
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        ...
        <label className="space-y-1 text-sm">
          ...
          <select
            ...
            onChange={(event) => setQualityProfile(event.target.value as MediaQualityProfile)}
            className="min-h-11 w-full rounded-lg border border-cream/[0.08] bg-cream/[0.04] px-3 py-2 text-sm text-foreground outline-none transition focus:border-accent/55 focus:bg-cream/[0.04] focus:ring-1 focus:ring-accent/25"
          >
      ...
      <section
        ...
        aria-live="polite"
      >
        <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-accent">Request summary</p>
        <dl className="mt-2 grid gap-x-4 gap-y-2 text-sm sm:grid-cols-2">
          <div>
            <dt className="text-xs text-muted">Title and scope</dt>
            <dd className="mt-0.5 font-medium text-foreground">
              {mediaType === "tv" ? `TV / ${describeTvSelection(selection)}` : "Movie"}
            </dd>
          </div>
          <div>
            <dt className="text-xs text-muted">Destination</dt>
            <dd className={selectedPath ? "mt-0.5 break-all font-medium text-foreground" : "mt-0.5 font-medium text-accent-wine"}>
              {destinationLabel}
            </dd>
          </div>
          <div>
            <dt className="text-xs text-muted">Preferred quality</dt>
            <dd className="mt-0.5 font-medium text-foreground">{selectedQuality?.label ?? qualityProfile}</dd>
          </div>
          <div>
            <dt className="text-xs text-muted">Monitoring</dt>
            <dd className="mt-0.5 font-medium text-foreground">{monitored ? "Monitor for missing content" : "Do not monitor"}</dd>
          </div>
        </dl>
        ...
        {downloadNow && !selectedPath ? (
          <div className="mt-2 flex flex-wrap gap-x-1 rounded-lg border border-accent-wine/30 bg-accent-wine/10 px-3 py-2 text-xs leading-5 text-foreground">
            <span>Download now needs an active {mediaType === "tv" ? "TV" : "movie"} destination.</span>
            <Link href="/settings/storage" className="font-semibold text-accent underline underline-offset-2">
