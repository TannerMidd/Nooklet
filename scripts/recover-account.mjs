...
function databasePathFromEnvironment() {
  ...
  if (!value || value === ":memory:") throw new Error("DATABASE_URL must point to an on-disk SQLite database.");
...
function requestedEmail() {
  ...
  if (index >= 0 && !value) throw new Error("Pass an email after --email.");
  return value || null;
}
...
async function main() {
  ...
  try {
    ...
    const user = email
      ? database.prepare("SELECT id, email, display_name AS displayName FROM users WHERE lower(email) = ?").get(email)
      : (() => {
          const admins = database.prepare("SELECT id, email, display_name AS displayName FROM users WHERE role = 'admin' AND is_disabled = 0").all();
          if (admins.length !== 1) {
            throw new Error("More than one active administrator exists. Re-run with --email <account-email>.");
          }
          return admins[0];
        })();

    if (!user) throw new Error(`No Nooklet account matches ${email}.`);
