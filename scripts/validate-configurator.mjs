...
function extractPowerShellPayload(command, name) {
  const match = command.match(
    new RegExp(String.raw`\$${name} = '([A-Za-z0-9+/=]+)'`),
  );
  assert.ok(match, `Expected ${name} payload.`);
  return Buffer.from(match[1], "base64").toString("utf8");
}

const generatedEnvironment = extractPowerShellPayload(
  windowsSetup.command,
  "GeneratedEnv",
);
const generatedOverride = extractPowerShellPayload(
  windowsSetup.command,
  "GeneratedOverride",
);
...
assert.match(
  ...
  /^DOWNLOAD_ENGINE_DIR=\/downloads\/nooklet-engine$/m,
);
...
assert.deepEqual(generatedHostPaths, [
  "D:/Media/Movies",
  "E:/Media/TV",
  "F:/Nooklet/Downloads",
]);
...
`,
      ...
      assert.match(
        exactRetry.stdout,
        /Reusing matching docker-compose\.override\.yml/,
      );
