...
function watchWorkerBundle() {
  if (process.env.NOOKLET_WATCH_WORKER_ENTRY !== "true") return;

  workerEntryWatcher = watch(path.dirname(workerEntry), { persistent: false }, (_event, filename) => {
    if (filename && filename.toString() !== path.basename(workerEntry)) return;
    if (workerReloadTimer) clearTimeout(workerReloadTimer);
...
async function bootstrap() {
  ...
  if (migrationProcess === migration) migrationProcess = undefined;
  if (shuttingDown) {
    finishShutdownIfReady();
    return;
  }
  if (result.code !== 0) {
    throw new Error(`database bootstrap exited with ${result.signal ?? result.code ?? "unknown"}`);
  }
