...
function verifyMigrations() {
    ...
    const missingMigrations = journal.entries
        .map((entry) => entry?.tag)
        .filter(
            (tag) => typeof tag !== "string" || !existsSync(path.join(drizzleRoot, `${tag}.sql`)),
        );

    if (missingMigrations.length > 0) {
        throw new Error(
            `Standalone output is missing migration SQL for: ${missingMigrations.join(", ")}`,
        );
    }
}
...
if (!existsSync(standaloneRoot) || !statSync(standaloneRoot).isDirectory()) {
    throw new Error(
        "Next.js standalone output was not created; refusing to publish an unverified build.",
    );
}
...
console.log(
    `Standalone artifact sanitized and verified (${removed.length} path${removed.length === 1 ? "" : "s"} removed).`,
);
