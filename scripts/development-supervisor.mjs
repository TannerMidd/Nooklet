...
function startWorkerSupervisor() {
    if (shuttingDown || !workerBundleReady) {
        return;
    }
    ...
    child.once("close", (code, signal) => {
        if (workerSupervisorProcess === child) {
            workerSupervisorProcess = undefined;
        }

        if (shuttingDown || plannedWorkerStops.has(child)) {
            return;
        }
...
async function bootstrap() {
    ...
    workerBuildContext = await context(
        await createWorkerBuildOptions({
            root,
